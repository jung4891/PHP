    ...
    <table width="100%" height="100%" cellspacing="0" cellpadding="0" style="padding:30px 30px 30px 30px" >
        <tr>
            ...
            <table width="100%" border="0" style="margin-top:50px; margin-bottom: 50px;">
						...
						foreach ($view_val2 as $item) {?>
								...
								<tr>
									...
									<td width="20%" align="left" class="t_border" style="padding-left:10px;">
										<?php if($item['product_type'] == "hardware"){
													echo "하드웨어"; 
											}else if($item['product_type'] == "software"){
												echo "소프트웨어"; 
											}else{
												echo "전체" ;
											}
			 ...
			 <tr>
				<td>
					<input type="button" class="basicBtn" value="통합유지보수" style="cursor:pointer;" onclick="integration_maintain_add();"/>
					<span style="float:right;">
						<img src="<?php echo $misc;?>img/btn_list.jpg" width="64" height="31" style="cursor:pointer;" onclick="location.href='<?php echo site_url();?>/maintain/integration_maintain_list'"/>
...
// 수정하러가기
function modify(){
	location.href="<?php echo site_url();?>/maintain/integration_maintain_input?seq=<?php echo $_GET['seq']; ?>";
}
...
function integration_maintain_add(){
	...
	if($('input:checkbox[name="product_row"]:checked').length > 0){
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "<?php echo site_url(); ?>/maintain/integration_maintain_add",
			dataType: "json",
			async: false,
			data: {
