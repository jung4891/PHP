...
if($mystep != ""){
   if(isset($approval_line[((int)$mystep+1)])){
      if($approval_line[((int)$mystep+1)]['approval_status'] == "" && $my_appoval_status == "Y" ){
         $approval_cancel_status="Y";
      }
   }
}
if(!empty($cur_approval_line)){
   if($cur_approval_line['step'] == ((int)$mystep+1)){
      $next_approval_line = $cur_approval_line;
   }
}
?>
<style>
                              ...
                              <div style="font-family:Noto Sans KR;">
                                 ...
                                 <?php
                                    if($type == "standby"){
                                       ...
                                       echo '<input type="button" class="basicBtn2" value="인쇄/PDF" onclick="print();" />';
                                       echo '<input type="button" class="basicBtn2" value="목록" onclick="list_view();" />';
                                    }else if($type == "progress"){
                                       ...
                                       if($_GET['type2'] != "002"){
                                          echo '<input type="button" class="basicBtn" style="margin-right:5px;" value="결재선" onclick="select_approval_modal();" />';
                                          echo '<input type="button" class="basicBtn2" value="참조자" onClick="select_user('."'referrer'".');"/>';
                                          echo '<input type="button" class="basicBtn2" value="결재복원" onClick="restore_approval_modal();" />';
                                       }
                                       echo '<input type="button" class="basicBtn2" value="';
                                       ...
                                       echo '<input type="button" class="basicBtn2" value="삭제" onclick="delete_doc();"/>';
                                       echo '<input type="button" class="basicBtn2" value="진행현황" onclick="progressStatus();" />';
                                       echo '<input type="button" class="basicBtn2" value="인쇄/PDF" onclick="print();" />';
                                       echo '<input type="button" class="basicBtn2" value="목록" onclick="list_view();" />';
                                    }
                                             ...
                                             <?php
                                                if($view_val['file_realname'] != ""){
                                                   ...
                                                   for($i=0; $i<count($file); $i++){
                                                      echo $file[$i];
                                                      echo "<a href='{$misc}upload/sales/electronic_approval/{$file_url[$i]}' download='{$file[$i]}'> <img src='{$misc}img/download.svg' style='width:15px;vertical-align:middle;cursor:pointer;margin-left:10px;'></a><br>";
                        ...
                        <div class="search-modal-content" style='height:auto; min-height:400px;overflow: auto;'>
                           <h2>결재선변경</h2>
                              ...
                              <div style="margin-top:5px;float:right;" >
                                 <input type ="button" class="basicBtn" value="변경" style="width:61px;" onClick="changeApproverLine();">
                                 <img src="<?php echo $misc;?>img/btn_cancel.jpg" style="cursor:pointer;" border="0" onClick="closeModal();"/>
                                 <!-- <img src="<?php echo $misc;?>img/btn_ok.jpg" style="cursor:pointer;float:right;margin-top:5px;" border="0" onClick="saveApproverLineModal();"/><br> -->
                              </div>
                        </div>
                     </div>
                        ...
                        <div class="search-modal-content" style='height:auto; min-height:400px;overflow: auto;'>
                           <h2>결재복원</h2>
                              ...
                              <div style='float:right;margin-top:5px;' >
                                 <input type='button' class="basicBtn" style='margin-right:5px;' value='결재복원' onclick="restore_approval();" />
                                 <input type='button' class="basicBtn2" value='결재회수' onclick="approval_withdraw();" />
                                 <img src="<?php echo $misc;?>img/btn_cancel.jpg" style="cursor:pointer;" border="0" onClick="closeModal();"/><br>
                              </div>
                        </div>
                     </div>
                     <!--내용-->
                              ...
                              <tr>
                                 <td class="answer2">
                                    ...
                                    <div id ="answer<?php echo $item['seq'] ;?>" style="display:none;">
                                       <textarea id="comment<?php echo $item['seq'] ;?>" cols="130" rows="5" class="input_answer1"><?php echo nl2br(str_replace(" ", "&nbsp;", htmlspecialchars($item['contents']))) ?></textarea>
...
<script>
   ...
   $.ajax({
      type: "POST",
      cache: false,
      url: "<?php echo site_url(); ?>/approval/approval_check_date_update",
      dataType: "json",
      async :false,
      data: {
   ...
   function saveUserModal(){
      ...
      $.ajax({
         type: "POST",
         cache: false,
         url: "<?php echo site_url(); ?>/approval/electronic_approval_doc_modify_action",
         dataType: "json",
         async: false,
         data: {
            type:1,
   ...
   function attach_view(seq){
      window.open('<?php echo site_url();?>/approval/electronic_approval_doc_preview?seq='+seq,'_blank',"width = 1200, height = 1000, top = 100, left = 400, location = no,status=no,status=no,toolbar=no,scrollbars=no");
      ...
      if(t == '1'){
         $.ajax({
            ...
            success: function (data) {
               if(data){
                  alert("결재가 저장되었습니다.");
                  location.href ="<?php echo site_url();?>/approval/electronic_approval_list?type=<?php echo $_GET['type']; ?>";
               }else{
                  alert("결재저장 실패!");
               }
            }
         ...
         if(confirm("결재 취소 하시겠습니까?")){
            $.ajax({
               ...
               async :false,
               data: {
                  seq : "<?php echo $my_approval_line_seq ;?>",
                  approval_form_seq : $("#approval_form_seq").val(),
                  next_seq : $("#next_approver_line_seq").val(),
                  approval_status: '',
                  approval_opinion: '',
   ...
   // $("#formLayoutDiv").find($("select")).prop('disabled', true);
   // $("#formLayoutDiv").find($("input")).css('cursor', "none");

   //개인보관함 팝업
   function personal_storage(){
      window.open("<?php echo site_url(); ?>/approval/electronic_approval_personal_storage_popup?seq=<?php echo $seq; ?>", "popup_window", "width = 500, height = 500, top = 100, left = 400, location = no,status=no,status=no,toolbar=no,scrollbars=no");
   }
   ...
   //재기안하러 이동!
   function reApproval(){
      location.href = "<?php echo site_url(); ?>/approval/electronic_approval_doc_modify?seq=<?php echo $_GET['seq']; ?>&type=1";
   }

   //수정하러 
   function modifyApproval(){
      location.href = "<?php echo site_url(); ?>/approval/electronic_approval_doc_modify?seq=<?php echo $_GET['seq']; ?>&type=0";
   }
   ...
   function list_view(){
      ...
      if( type == "request" ){
         location.href = "<?php echo site_url(); ?>/approval/electronic_approval_doc_list?type="+type;
      }else{
   ...
   function commentSave(type,seq){
      if(type == 0){//등록
         $.ajax({
            type: "POST",
            cache: false,
            url: "<?php echo site_url(); ?>/approval/commentSave",
            dataType: "json",
            async: false,
            data: {
               type : type,
   ...
   function restore_approval(){
      if(confirm("결재 복원 하시겠습니까?")){
         ...
         $.ajax({
            type: "POST",
            cache: false,
            url: "<?php echo site_url(); ?>/approval/restore_approval",
            dataType: "json",
            async: false,
            data: {
               seq : <?php echo $seq ?>,
   ...
   <?php if($type == "admin"){?>
      function changeApproverLine(){
         ...
         $.ajax({
            type: "POST",
            cache: false,
            url: "<?php echo site_url(); ?>/approval/changeApproverLine",
            dataType: "json",
            async: false,
            data: {
               seq : <?php echo $seq ?>,
      ...
      if(security_status == "N"){
         if(confirm("보안설정 하시겠습니까?")){
            $.ajax({
               type: "POST",
               cache: false,
               url: "<?php echo site_url(); ?>/approval/security_setting",
               dataType: "json",
               async: false,
               data: {
                  seq: <?php echo $seq; ?> ,
   ...
   function delete_doc(){
      if(confirm("결재 문서를 삭제하시겠습니까?")){
         $.ajax({
            type: "POST",
            cache: false,
            url: "<?php echo site_url(); ?>/approval/electronic_approval_doc_delete",
            dataType: "json",
            async: false,
            data: {
               seq: <?php echo $seq; ?>
            ...
            success: function (data) {
               if (data) {
                  alert("결재문서 삭제 완료");
                  location.href='<?php echo site_url();?>/approval/electronic_approval_list?type=admin'
               } else {
   ...
   //인쇄 && pdf
   function print(){
      window.open('<?php echo site_url();?>/approval/electronic_approval_doc_print?seq=<?php echo $seq ;?>','_blank');
   }
