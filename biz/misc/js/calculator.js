...
var calculate = {
   ...
   * @param taxtypeval    세금구분(8.98% = 0.08344 or 3.3% = 0.033)
   */
  wage: function (hear, dayworktime, monthworkday, tranningset, taxtypeval) {
    ...
    if (isTaxSet != 0) {
      ...
      var minusmoney = totalDateSum * taxtypeval;
  ...
  retirement: function (entryDt, calcDt, yearly, annual) {
    ...
    var startDt = new Date(entryDt);
    var endDt = new Date(calcDt);
  ...
  insurance: function (salary, workers, accident) {
    ...
    // ==================== 국민연금 ==================== //
    var pension_worker = Math.floor((setSalary * 0.045) / 10) * 10; // 근로자 부담금(월급여 * 근로자부담율(4.5%))
    var pension_business = Math.floor((setSalary * 0.045) / 10) * 10; // 사업자 부담금(월급여 * 사업주부담율(4.5%))
    ...
    // ==================== 건강보험 ==================== //
    var health_worker = Math.round((setSalary * 0.0343) / 10) * 10; // 월급여 * 근로자 부담율(3.43%)
    var health_business = Math.round((setSalary * 0.0343) / 10) * 10; // 월급여 * 사업주 부담율(3.43%)
    ...
    // ==================== 건강보험(장기요양) ==================== //
    var care_premium = Math.floor((health_premium * 0.1152) / 20) * 20; // 건강보험료 총액 * 11.52%
    ...
    // ==================== 고용보험 ==================== //
    // 실업급여요율
    var eiUnWorker = "0.008"; // 고용보험 실업급여 근로자
    var ei150lOwner = "0.0025"; // 고용보험 150미만 사업주
    var ei150hOwner = "0.0045"; // 고용보험 150이상 사업주
    var ei1000lOwner = "0.0065"; // 고용보험 1000미만 사업주
    var ei1000hOwner = "0.0085"; // 고용보험 1000이상 사업주

    if (setWorkers == 0) {
      //owner_total_text = "(150인 미만)";
      owner_total =
        Math.floor((setSalary * Number(ei150lOwner)) / 10) * 10 +
        Math.floor((setSalary * Number(eiUnWorker)) / 10) * 10;
    } else if (setWorkers == 1) {
      //owner_total_text = "(150이상(우선지원대상기업))";
      owner_total =
        Math.floor((setSalary * Number(ei150hOwner)) / 10) * 10 +
        Math.floor((setSalary * Number(eiUnWorker)) / 10) * 10;
    } else if (setWorkers == 2) {
      //owner_total_text = "(1,000인 이상)";
      owner_total =
        Math.floor((setSalary * Number(ei1000hOwner)) / 10) * 10 +
        Math.floor((setSalary * Number(eiUnWorker)) / 10) * 10;
    } else if (setWorkers == 3) {
      //owner_total_text = "(150인 이상 1,000인 미만)";
      owner_total =
        Math.floor((setSalary * Number(ei1000lOwner)) / 10) * 10 +
        Math.floor((setSalary * Number(eiUnWorker)) / 10) * 10;
    }
    // console.log(
    //   "\n사업주 부담액" +
    ...
    var worker_total = Math.floor((salary * eiUnWorker) / 10) * 10;
   ...
   * @param severance     0: 포함, 1: 미포함
   * @param taxFreeLb     비과세액
   * @param supportLb     부양가족수
   * @param annIncome     연봉
   */
  pay: function (division, severance, taxFreeLb, supportLb, annIncome) {
     ...
     * @type {number}
     */
    var salary_pension = salary.salary_pension(setDivision, minusPrice);
     ...
     * @type {number}
     */
    var salary_health = salary.salary_health(setDivision, minusPrice);
     ...
     * @type {number}
     */
    var salary_care = salary.salary_care(setDivision, salary_health);
     ...
     * @type {number}
     */
    var salary_insurance = salary.salary_insurance(setDivision, minusPrice);
   ...
   * @param annIncome     월급
   */
  salary: function (division, taxFreeLb, supportLb, annIncome) {
    var setDivision = division;
    var setTaxFreeLb = taxFreeLb;
    var setSupportLb = supportLb;
    var setAnnIncome = annIncome;
...
var salary = {
   ...
   * @returns {number}
   */
  salary_pension: function (division, minusPrice) {
    var pension = 0; // 국민연금

    if (division == 0) {
      // (연봉 - 비과세액) * 4.5%(0.045)
      pension = Math.floor((minusPrice * 0.045) / 10) * 10;
    } else {
      // (월급 - 비과세액) * 4.5%(0.045)
      pension = Math.floor((minusPrice * 0.045) / 10) * 10;
    }
   ...
   * @returns {number}
   */
  salary_health: function (division, minusPrice) {
    var medical = 0; // 건강보험

    if (division == 0) {
      // (연봉 - 비과세액) * 3.43%(0.0343)
      medical = Math.floor((minusPrice * 0.0343) / 10) * 10;
    } else {
      // (월급 - 비과세액) * 3.43%(0.0343)
      medical = Math.round((minusPrice * 0.0343) / 10) * 10;
    }
    return medical;
  },
  /**
   ...
   * @returns {number}
   */
  salary_care: function (division, medical) {
    var care = 0; // 장기요양

    if (division == 0) {
      // 건강보험 * 11.52%(0.1152)
      care = Math.floor((medical * 0.1152) / 10) * 10;
    } else {
      // 건강보험 * 11.52%(0.1152)
      care = Math.floor((medical * 0.1152) / 10) * 10;
    }
   ...
   * @returns {number}
   */
  salary_insurance: function (division, minusPrice) {
    var employ = 0; // 고용보험

    if (division == 0) {
      // (연봉 - 비과세액) * 0.8%(0.008)
      employ = Math.floor((minusPrice * 0.008) / 10) * 10;
    } else {
      // (월급 - 비과세액) * 0.8%(0.008)
      employ = Math.floor((minusPrice * 0.008) / 10) * 10;
    }
