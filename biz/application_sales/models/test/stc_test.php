...
//		$this->user_id = $this->phpsession->get( 'id', 'stc' );
  ...
  function user_list($sn) {
	...
	function order_completed_serial_count($sn) {
		$sql = "
		select
			count(sf.seq) as ucount
		from
			(SELECT *,
				(SELECT sum(percentage) FROM sales_forcasting_bill WHERE forcasting_seq =s.seq AND TYPE='001' AND issuance_status='Y') AS bill_progress_step
			 FROM sales_forcasting AS s) sf,
	...
	function maintain_serial($sn) {
		...
		FROM (SELECT *,
		      (SELECT count(*) FROM sales_maintain_bill WHERE maintain_seq =s.seq AND TYPE='001' AND issuance_status='Y') AS bill_progress_step
		     	FROM sales_maintain AS s {$forcasting} ) sf
			   join
			   ...
			   on p.seq = sp.product_code
				 WHERE sp.product_serial LIKE '%$sn%'
