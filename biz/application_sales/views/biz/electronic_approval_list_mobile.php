...
<?php if($type != "admin"){
		...
		<a class="menu_list" onclick ="moveList('standby')" style='color:<?php if($type == "standby"){echo "#0575E6";}else{echo "#B0B0B0";}?>'>결재대기함</a>
		<a class="menu_list" onclick ="moveList('progress')" style='color:<?php if($type == "progress"){echo "#0575E6";}else{echo "#B0B0B0";}?>'>결재진행함</a>
		<a class="menu_list" onclick ="moveList('completion')" style='color:<?php if($type == "completion"){echo "#0575E6";}else{echo "#B0B0B0";}?>'>완료문서함</a>
		<a class="menu_list" onclick ="moveList('back')" style='color:<?php if($type == "back"){echo "#0575E6";}else{echo "#B0B0B0";}?>'>반려문서함</a>
	 ...
	 for($i = $start_row; $i<$start_row+$end_row; $i++){
			if(!empty( $view_val[$i])){
				 ...
				 if($doc['approval_doc_hold'] == "N"){
						...
						echo "<td align='left' style='color:#1C1C1C;font-weight:bold;'>{$doc['approval_doc_name']} (보류)</td>";
				 }
...
<?php
	 ...
	 foreach($delegation as $doc){
				 ...
				 echo "<tr align='center' onclick='eletronic_approval_view({$doc['seq']})'>";
				 if($doc['approval_doc_hold'] == "N"){
						echo "<td align='left' style='color:#1C1C1C;font-weight:bold;'>{$doc['approval_doc_name']}(위임)</td>";
				 }else{
						echo "<td align='left' style='color:#1C1C1C;font-weight:bold;'>{$doc['approval_doc_name']} (보류)</td>";
				 }
				 echo "<td align='right' style='color:#1C1C1C;font-weight:bold;'>";
  ...
  <div id="search_div" style="height:auto;width:100%;background-color:#ffffff; display:none;border-radius:5px;">
    <div class="modal_contain" style="font-size:16px; color:#1C1C1C;font-weight:bold;">
      <table style="width:100%;padding:5%;" cellspacing="0">
				...
				<tr class="admin_tr">
      		<td align="left" colspan="2" height="40">문서상태</td>
      	</tr>
				<tr class="admin_tr">
					<td colspan="2">
      	...
      	<tr>
      		<td align="left" colspan="2" height="40">양식명</td>
      	</tr>
        <tr>
          <td colspan="2">
        ...
        <tr>
      	...
      	<tr>
      		<td align="left" colspan="2" height="40">기안자</td>
      	</tr>
        <tr>
          <td colspan="2">
	...
	<div style="padding-left:10px;padding-bottom:60px;">
		<span style="color:red;margin-right:5px;">*</span>결재문서 검색 시 우측 하단에 검색 아이콘을 눌러주세요.
	</div>
