...
if($this->cooperation_yn != 'Y') {
  ...
  if( $tech_lv == 1 ){
    if($view_val['writer'] != $name){
      echo "<script>alert('수정 권한이 없습니다.');history.go(-1);</script>";
      echo exit;
    }
  }else if ($tech_lv == 2){
...
<script language="javascript">
  ...
  function check_add(btn,tableNum) {
    ...
    // if($('#work_text_table'+tableNum).find($(".check"+nextTr)).length != 0){
      ...
      <td class="tdSolid"><textarea name="work_text'+trNum+'[]" onkeyup="xSize(this);"></textarea><img src="<?php echo $misc; ?>img/btn_minus_white.svg" style="cursor:pointer;width:23px;" onclick ="check_del(this)" /></td></tr>');
    // }else{
  ...
  function filedel(seq, filename) {
    if (confirm("정말 삭제하시겠습니까?") == true) {
      location.href = "<?php echo site_url(); ?>/tech/tech_board/tech_doc_filedel/" + seq + "/" + filename;
      return false;
    }
  }
  ...
  <script src="<?php echo $misc; ?>ckeditor/ckeditor.js"></script>
    <tr height="5%">
      ...
      <tr>
        <tr>
          <td align="right">
            <input style="margin-top:20px;margin-right:10px;margin-bottom:20px;width:80px;color:#333;" type="button" class="btn-common" value="임시저장" onClick="javascript:chkForm('N');return false;">
            <input style="margin-top:20px;margin-right:10px;margin-bottom:20px;width:60px;" type="button" class="btn-common btn-color4" value="취소" onClick="javascript:history.go(-1)">
            ...
            <input style="margin-top:20px;margin-right:40px;margin-bottom:20px;width:60px;" type="button" class="btn-common btn-color2" value="등록" onClick="javascript:chkForm('Y');return false;">
          </td>
        </tr>
          ...
          <table width="100%" height="100%" cellspacing="0" cellpadding="0">
            <tr>
              <td width="100%" align="center" valign="top">
                <form name="cform" action="<?php echo site_url();?>/tech/tech_board/tech_doc_input_action?type=<?php echo $_GET['type']; ?>" method="post" enctype="multipart/form-data">
              <!-- <form name="cform" action="<?php echo site_url();?>/tech/tech_board/tech_doc_input_action" method="post" enctype="multipart/form-data" onSubmit="javascript:chkForm();return false;"> -->
                  ...
                  <input type="hidden" id="customer_tmp" name="customer_tmp" value="<?php echo $view_val['customer']; ?>">
                  <input type="hidden" id="forcasting_seq" name="forcasting_seq" value="<?php echo $view_val['forcasting_seq']; ?>">
                  <input type="hidden" id="maintain_seq" name="maintain_seq" value="<?php echo $view_val['maintain_seq']; ?>">
                  ...
                  <tr>
                    <td>
                      <table id="input_table" class="list_tbl" width="100%" border="0" cellspacing="0" cellpadding="0">
                        ...
                        <tr>
                          ...
                          <td style="padding-left:10px;">
                            <div class="dropdown" id="dropdown">
                              ...
                              <input id="customerName" name ="customerName" type="text" class="input-common" style="width:200px;font-weight:bold;text-align:center;margin-right:5px;" value="<?php echo $view_val['customer'] ?>" readonly>
                              ...
                              <input type="hidden" id="customer" name="customer" value="<?php if($view_val['work_name']=="납품설치" || $view_val['work_name']=="미팅" || $view_val['work_name']=="데모(BMT)지원"){ echo $view_val['forcasting_seq']; }else{ if($view_val['maintain_seq'] == null ){ echo $view_val['forcasting_seq']; }else{ echo $view_val['maintain_seq']; } } ?>" style="border:none" readonly>
                              <!-- KI2 20210125 -->
                        ...
                        <tr>
                          ...
                          <td style="padding-left:10px;">
                            <input type="date" name="income_time" id="income_time" class="input-common" value="<?php echo substr($view_val['income_time'], 0, 10); ?>" <?php if(isset($schedule)){echo "readonly";} ?>>
                          </td>
                          <td align="center" class="row-color1" style="font-weight:bold;">작업종료일</td>
                          <td class="border-r" style="padding-left:10px;">
                            <input type="date" name="end_work_day" id="end_work_day" class="input-common" value="<?php
                              if($view_val['end_work_day'] == NULL){
                                echo substr($view_val['income_time'], 0, 10);
                              }else{
                                echo substr($view_val['end_work_day'], 0, 10);
                              }
                            ?>" <?php if(isset($schedule)){echo "readonly";} ?> >
                          </td>
                        </tr>
                        <tr>
                          ...
                          <td style="padding-left:10px;">
                            <!-- KI1 20210125 $schedule값이 있을 경우 불러오기 추가-->
                            <input name="project_name" id="project_name" class="input-common" value="<?php echo $view_val['project_name']; ?>" readonly>
                            <!-- KI2 20210125 -->
                          </td>
                        ...
                        for ($i = 0; $i < count($arr_manager); $i++) {
                          ...
                          // ki
                        ?>
                            <tr class="field" id="line_field_<?php echo $i; ?>">
                          ...
                          <td style="padding-left:10px;">
                            <!-- KI1 20210125 -->
                            <input type="text" name="customer_manager[]" id="customer_manager" class="input-common customer_manager" value="<?php echo $arr_manager[$i] ?>">
                          ...
                          <td class="border-r" style="padding-left:10px;">
                            <input type="text" name="manager_mail[]" id="manager_mail" class="input-common manager_mail" value="<?php echo $arr_mail[$i] ?>" style="width:calc(100% - 35px);">
                            <?php if ($i == 0) { ?>
                            ...
                            <?php } else { ?>
                            <img src='<?php echo $misc; ?>img/btn_minus_white.svg' style='cursor:pointer;width:23px;' onclick='javascript:main_list_del(<?php echo $i; ?>);' />
                            <?php } ?>
                          </td>
                        ...
                        <tr>
                          ...
                          <td style="padding-left:10px;">
                            <input type="eu-time" name="start_time" id="start_time" class="input-common" value="<?php echo substr($view_val['start_time'],0,5); ?>" autocomplete="off"  <?php if(isset($schedule)){echo "readonly";} ?>/>
                          </td>
                          <td align="center" class="row-color1" style="font-weight:bold;">종료시간</td>
                          <td class="border-r" style="padding-left:10px;">
                            <input type="eu-time" name="end_time" id="end_time" class="input-common" value="<?php echo substr($view_val['end_time'],0,5); ?>" autocomplete="off"  <?php if(isset($schedule)){echo "readonly";} ?>/>
                          </td>
                        </tr>
                      ...
                      <?php }
                        ...
                        <tr>
                          ...
                          <td style="padding-left:10px;">
                            <input type="text" name="engineer" id="engineer" class="input-common" onclick="test2();" value="<?php echo $view_val['engineer']; ?>" onclick="test2();" readonly>
                            <input type="hidden" name="engineer_seq" id="engineer_seq" class="input-common" value="<?php echo $view_val['engineer_seq']; ?>" />
                          </td>
                          <td align="center" class="row-color1" style="font-weight:bold;">지원방법</td>
                          <td class="border-r" style="padding-left:10px;">
                            <select name="handle" id="handle" class="input2" style="height:23px;">
                        ...
                        <tr>
                          ...
                          <td align="left" colspan="2" style="padding-left:10px;">
                            ...
                            <input type="hidden" name="manufacturer" id="manufacturer" class="input2_blue" value="<?php echo $view_val['manufacturer']; ?>" readonly>
                            <input type="hidden" name="duplication_yn" id="duplication_yn" class="input2_blue" value="<?php echo $view_val['duplication_yn']; ?>" readonly>
                          ...
                          <ul id="sortable">
                            <?php
                              ...
                              $manufacturer = array();
                              if(trim($view_val['manufacturer']) == "" || $view_val['manufacturer'] == null){//produce_seq가 후에 생겨서 빈값인 경우
                                for($i=0; $i<count($produce); $i++){
                                  $manufacturer[$i]= '';
                                }
                              }else{
                              ...
                              if(trim($view_val['duplication_yn']) == "" || $view_val['duplication_yn'] == null){//produce_seq가 후에 생겨서 빈값인 경우
                                for($i=0; $i<count($produce); $i++){
                                  $duplication_yn[$i]= '';
                                }
                              }else{
                              ...
                              for($i=1; $i<=count($produce); $i++){
                                ...
                                echo '<span class="produce_seq" style="display:none;">'.$produce_seq[($i-1)].'</span>';

                                echo '<span style="font-weight:bold;"></span><span class="produce">'.$produce[($i-1)].'</span>';
                                echo '<span style="font-weight:bold;"> / </span><span class="manufacturer">'.$manufacturer[($i-1)].'</span>';
                                echo '<span style="font-weight:bold;"> / </span><span class="host">'.$host[($i-1)].'</span>';
                                echo '<span style="font-weight:bold;"> / </span><span class="version">'.$version[($i-1)].'</span>';
                                echo '<span style="font-weight:bold;"> / </span><span class="hardware">'.$hardware[($i-1)].'</span>';
                                echo '<span style="font-weight:bold;"> / </span><span class="license">'.$license[($i-1)].'</span>';
                                echo '<span style="font-weight:bold;"> / </span><span class="serial">'.$sn[($i-1)].'</span>';
                                echo '<span style="font-weight:bold;"> / </span><span class="duplication_yn">'.$duplication_yn[($i-1)].'</span>';

                                echo '</span>&nbsp;&nbsp;<input type="button" value="×" style="color:red;cursor:pointer;border:none;background-color:#FFEDED;" onclick="produceRemove('.$i.')" /></li>';
                              }
                            ?>
                            </ul>
                        </td>
                        ...
                        <tr>
                          ...
                          <td colspan="3" class="border-r" style="padding-left:10px;">
                            <input type="text" name="subject" id="subject" class="input-common" value="<?php echo $view_val['subject']; ?>" />
                          </td>
                        </tr>
                        ...
                        <tr>
                          ...
                          <td style="padding-left:10px;">
                            <input type="date" name="work_day_s[]" id="work_day_s" class="input-common" style="width:48%;" size="8" value="<?php echo $date[0]; ?>">&ensp;
                            <input type="date" name="work_day_e[]" id="work_day_e" class="input-common" style="width:48%;" size="8" value="<?php if(!empty($date[1])){ echo $date[1]; }else{ echo $date[0]; } ?>">
                          </td>
                          <td align="center" class="row-color1" style="font-weight:bold;">1. 지원시간</td>
                          <td class="border-r" style="padding-left:10px;">
                            <input type="eu-time" name="work_time_s[]" id="work_time_s" class="input-common" autocomplete="off" style="width:48%" size="2" value="<?php echo $time[0]; ?>">&ensp;
                            <input type="eu-time" name="work_time_e[]" id="work_time_e" class="input-common" autocomplete="off" style="width:48%" size="2" value="<?php echo $time[1]; ?>">
                          </td>
                        </tr>
                        <tr>
                        ...
                        <tr class="addRow">
                          <td colspan="4" class="border-l border-r" style="padding:10px 20px 10px 20px;">
                            <textarea rows="5" style="width:calc(100% - 35px);float:left;" name="work_text[]" id="work_text" onkeyup="resize(this)" value="<?php echo $process_txt[$i]; ?>"><?php echo $process_txt[$i]; ?>
                            </textarea>
                    ...
                    ?>
                        ...
                        <tbody id ="periodic">
                          <tr>
                            <td colspan="4" height="40" align="center" style="font-weight:bold;" class="border-l border-r">
                              ...
                              ?>
                              <table id="work_text_table<?php echo ($a+1) ;?>" class ="work_text_table" width=100% border=1 style="display:none;border-collapse:collapse;border:none;">
                                <tr>
                                      ...
                                      $process_text1 = explode('#*',$process_text[$i]); //점검 내용 별로 나누기
                                        if($i <> 1){ //기타 특이사항을 제외한 나머지
                                            ...
                                            for($j=1; $j<count($process_text1); $j++){
                                              if($j==1){
                                                  echo '<tr class="check'.($i-1).'"><td class="tdSolid" rowspan="'.floor((count($process_text1)-1)/3).'"><input name="work_text'.($i-1).'[]" value="'.$process_text1[$j].'" style="width:80%;border:none;"></input><img src="'.$misc.'img/btn_minus_white.svg" style="cursor:pointer;width:23px;" class="adddelbtn" onclick="trRemove(this,'.($a+1).');" /></td>'; //cpu, 메모리
                                                  ...
                                                  if($j!=4){
                                                    if($j%3==0){
                                                      ...
                                                      if(strpos($process_text1[$j],"::") === false){
                                                        if($process_text1[$j] == "::"){
                                                          ...
                                                          echo '<td class="tdSolid"><input type="hidden" name="work_text'.($i-1).'[]" value="::"><textarea name="work_text'.($i-1).'[]" onkeyup="xSize(this);">'.$text[1].'</textarea><img src="'.$misc.'img/btn_minus_white.svg" style="cursor:pointer;width:23px;" class="tdDivisionBtn" onclick="tdMerge(this,'.($basicFormIdx-1).');" /></td>';
                                                        }else{
                                                          echo '<td class="tdSolid" colspan="2"><textarea name="work_text'.($i-1).'[]" onkeyup="xSize(this);">'.$process_text1[$j].'</textarea><img src="'.$misc.'img/dashboard/dash_plus.svg" style="cursor:pointer;" class="tdDivisionBtn" onclick="tdDivision(this,'.($basicFormIdx-1).');" /></td>';
                                                        }
                                                      }else{
                                                        $text = explode('::',$process_text1[$j]);
                                                        echo '<td class="tdSolid"><textarea name="work_text'.($i-1).'[]" onkeyup="xSize(this);">'.$text[0].'</textarea></td>';
                                                    ...
                                                    echo '<td class="tdSolid"><textarea name="work_text'.($i-1).'[]" onkeyup="xSize(this);">'.$process_text1[$j].'</textarea><img src="'.$misc.'img/dashboard/dash_plus.svg" onclick ="check_add(this,'.($a+1).')" style="cursor:pointer;" /></td></tr>';
                                                  }
                                                  ...
                                                  if($j%3==2){
                                                    ...
                                                    if(strpos($process_text1[$j],"::") === false){
                                                      if($process_text1[$j] == "::"){
                                                        ...
                                                        echo '<tr class="check'.($i-1).'"><td class="tdSolid" colspan="2"><textarea name="work_text'.($i-1).'[]" onkeyup="xSize(this);">'.$process_text1[$j].'</textarea><img src="'.$misc.'img/dashboard/dash_plus.svg" style="cursor:pointer;" class="tdDivisionBtn" onclick="tdDivision(this,'.($basicFormIdx-1).');" /></td>';
                                                      }
                                                    }else{
                                                      $text = explode('::',$process_text1[$j]);
                                                      echo '<tr class="check'.($i-1).'"><td class="tdSolid"><textarea name="work_text'.($i-1).'[]" onkeyup="xSize(this);">'.$text[0].'</textarea></td>';
                                                    ...
                                                    echo '<td class="tdSolid"><textarea name="work_text'.($i-1).'[]" onkeyup="xSize(this);">'.$process_text1[$j].'</textarea><img src="'.$misc.'img/btn_minus_white.svg" style="cursor:pointer;width:23px;" onclick ="check_del(this)" /></td></tr>';
                                                  }
                                              }
                                            }
                                        }
                                      ...
                                      if($i == 1){
                                        ...
                                        for($j=1; $j<count($process_text1); $j++){
                                          if($j+1 <> count($process_text1)){
                                              echo '<tr class="check0" class="check'.(count($process_text)-1).'"><td class="tdSolid"><input style="border:none;" name="work_text0[]" value="'.$process_text1[$j].'"></input></td>';
                                          }else{
                        ...
                        <?php if($view_val['failure_contents'] != '') {
                                ...
                                foreach($failure_data as $fd) {
                                  ...
                                  <tr class="err_row">
                                    <td width="15%" height="40" align="center" bgcolor="f8f8f9" style="font-weight:bold;" class="row-color1 border-l">
                                      <input type="text" class="input-common failure_title" style="text-align:center;font:'Noto Sans KR', sans-sans-serif;font-weight:bold;font-size:12px;" name="failure_title[]" value="<?php echo $failure_title; ?>">
                                    </td>
                                    <td colspan="3" style="font-weight:bold;padding: 12px;" class="border-r">
                                      <input type="text" name="failure_content[]" id="failure_content[]" class="input-common" style="width:98%;" value="<?php echo  $failure_content; ?>" />
                              <?php if($i == 0) { ?>
                                    <img src="<?php echo $misc; ?>img/dashboard/dash_plus.svg" onclick="failure_add();" style="cursor:pointer;vertical-align:middle;" /></td>
                              <?php } else { ?>
                                    <img src='<?php echo $misc; ?>img/btn_minus_white.svg' style='cursor:pointer;width:23px;vertical-align:middle;' onclick="failure_del(this);"/></td>
                              <?php } ?>
                                  </tr>
                          <?php
                                  $i++;
                        ...
                        <tr>
                          ...
                          <td colspan="3" class="border-r" style="padding-left:10px;">
                            ...
                            <input type="text" name="comment" id="comment" class="input-common" value="<?php echo  $view_val['comment']; ?>" />
                          </td>
                        </tr>
                        ...
                        <tr>
                          ...
                          <td colspan="3" class="border-r" style="padding-left:10px;">
                            ...
                            <input type="text" name="comment" id="comment" class="input-common" value="<?php if (!empty($request)){ echo $request['contents']; } ?>" />
                          </td>
                        </tr>
                        <tr>
                          ...
                          <td colspan="3" class="border-r" style="padding-left:10px;">
                            ...
                            <input type="text" name="issue" id="issue" class="input-common" value="<?php if (!empty($issue)){echo $issue['contents'];}?>" />
                          </td>
                        </tr>
                        <tr>
                          ...
                          <td colspan="3" class="border-r" style="padding-left:10px;">
                            ...
                            <input type="text" name="bug" id="bug" class="input-common" value="<?php if (!empty($bug_val)){ echo $bug_val['contents']; } ?>" />
                          </td>
                        </tr>
                        <tr>
                          <td align="center" class="row-color1 border-l" style="font-weight:bold;">확인서명</td>
                          <td style="padding-left:10px;">
                            <input type="checkbox" id="sign_consent" name="sign_consent" value="<?php echo $view_val['sign_consent']; ?>">서명동의
                        ...
                        <tr>
                          ...
                          <td colspan="3" class="border-r" style="padding-left:10px;">
                              <div class="box-file-input s_file_input_box" style="margin:10px;"><span style="margin-right:10px;" class="file_span">
                                <?php if ($view_val['file_changename']) { ?>
                                  <a href="<?php echo site_url(); ?>/tech/tech_board/tech_doc_download/<?php echo $seq; ?>/<?php echo $view_val['file_changename']; ?>">
                                    <?php echo $view_val['file_realname']; ?></a>
                                    &emsp;<input type="button" value="×" style="color:red;cursor:pointer;border:none;background-color:#FFEDED;" onclick="fileRemove()" />
                                  <?php } ?>
                              </span>
                        ...
                        <tr>
                          ...
                          <td colspan="3" class="border-r" style="padding-left:10px;">
                            <input type="text" name="hashtag" id="hashtag" class="input-common" value="" placeholder="#홍길동  #두리안정보기술" value="<?php
                            if(!empty($hashtag)) {
                              for($i =  0; $i < count($hashtag); $i++) {
                                echo '#'.$hashtag[$i]['hashtag_name'].' ';
                              }
                            }
                            ?>" />
                          </td>
                ...
                for (j = 1; j < process_text1.length; j++) {
                  if (j == 1) {
                    ...
                    if (j % 3 == 2) {
                      ...
                      txt += '>정상<input type="radio" name="produce' + tablenum + "_" + ( - 1) + '" value="abnormal" onclick="normalCheck(this);"'
                      if (process_text1[j] == "abnormal") {
                        txt += 'checked="checked"';
                      }
                      txt += '>비정상</div></td>';
                    } else if (j % 3 == 1) {
                      txt += '<td class="tdSolid"><textarea name="work_text' + (i - 1) + '[]" onkeyup="xSize(this);">' + process_text1[j] + '</textarea></td><td width="3%" class="t_border adddelbtn"><input type="hidden" name="del_ck" ></td><td align="center;" class ="adddelbtn"><img src="<?php echo $misc ;?>img/btn_minus_white.svg" style="cursor:pointer;width:23px;" onclick ="check_del(this)" /></td></tr>';
                    }
                  }
                }
              }
