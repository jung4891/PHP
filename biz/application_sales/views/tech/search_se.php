...
?>
                 <tr bgcolor="f8f8f9" class="t_top">
                    ...
                    <input type="hidden" name="user_seq"  value="<?php echo $entry->seq?>">
                  </tr>
<?php
			}
		?>
...
for(i = 0; i<len-1;i++){
        ...
        total_dev_name += document.getElementsByName('user_duty')[i].value + name_check;
				total_dev_seq += document.getElementsByName('user_seq')[i].value + seq_check;
        }
...
opener.document.cform.engineer_seq.value=total_dev_seq.substr(0,(total_dev_seq.length-1));
self.close();
}
</script>
