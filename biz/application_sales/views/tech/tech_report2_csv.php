...
$income_time = substr($view_val['income_time'],0,10);
	...
	if($_GET['pdf_type'] == 1){//상세보기 및 서명 전송
		...
		// $h$headers .= "Bcc: sylim@durianit.co.kr" ."\r\n";eaders .= 'Message-ID: <' . $i . ">";

		$result = mail($to, $subject, chunk_split(base64_encode($message)), $headers);
	}else{//pdf 전송
		exec("/usr/local/bin/wkhtmltopdf --encoding UTF-8 -L 19mm -B 10mm  --footer-html ".site_url()."/tech/tech_board/tech_doc_pdf_logo?seq={$seq} ".site_url()."/tech/tech_board/tech_doc_pdf?seq={$seq} /var/www/html/stc/misc/upload/tech/tech/techpdf.pdf");
