...
<input type="hidden" name="work_text" id="work_text" value="" />
<input type="hidden" name="seq" id="seq" value="<?php echo $view_val[0]['seq'] ;?>" />
<h2 align="center">
제품별 점검항목 수정
</h2>
<div style="margin:10px 0 10px 0 ;"><span class="title">제품명</span><input type="text" style="border: 1px solid #B0B0B0; border-radius: 19px; opacity: 1;outline: none" name="product_name" id="product_name" class="input2" value="<?php echo $view_val[0]['product_name'] ;?>" ></div>
<?php
            ...
            $process_text1 = explode('#*',$process_text[$i]); //점검 내용 별로 나누기
              if($i <> 1){ //기타 특이사항을 제외한 나머지
                  ...
                  for($j=1; $j<count($process_text1); $j++){
                    if($j==1){
                        echo '<tr class="check'.($i-1).'"><td class="tdSolid" rowspan="'.floor((count($process_text1)-1)/3).'"><input name="work_text'.($i-1).'[]" value="'.$process_text1[$j].'" style="width:70%;word-wrap:break-word;"></input><img src="'.$misc.'img/dashboard/btn/icon_minus.svg" style="cursor:pointer;" class="adddelbtn" onclick="trRemove(this,'.($a+1).');" /></td>'; //cpu, 메모리
                        ...
                        if($j!=4){
                          if($j%3==0){
                            ...
                            if(strpos($process_text1[$j],"::") === false){
                              if($process_text1[$j] == "::"){
                                ...
                                echo '<td class="tdSolid"><input type="hidden" name="work_text'.($i-1).'[]" value="::"><textarea name="work_text'.($i-1).'[]" onkeyup="xSize(this);">'.$text[1].'</textarea><img src="'.$misc.'img/dashboard/btn/icon_minus.svg" style="cursor:pointer;" class="tdDivisionBtn" onclick="tdMerge(this,'.($basicFormIdx-1).');" /></td>';
                              }else{
                                echo '<td class="tdSolid" colspan="2"><textarea name="work_text'.($i-1).'[]" onkeyup="xSize(this);">'.$process_text1[$j].'</textarea><img src="'.$misc.'img/dashboard/btn/icon_plus.svg" style="cursor:pointer;" class="tdDivisionBtn" onclick="tdDivision(this,'.($basicFormIdx-1).');" /></td>';
                              }
                            }else{
                              $text = explode('::',$process_text1[$j]);
                              echo '<td class="tdSolid"><textarea name="work_text'.($i-1).'[]" onkeyup="xSize(this);">'.$text[0].'</textarea></td>';
                          ...
                          echo '<td width="3%" class="t_border adddelbtn"><input type="hidden" name="del_ck" id="del_ck" class="input2"></td><td align="center" class ="adddelbtn" ><img src="'.$misc.'img/dashboard/btn/icon_plus.svg" onclick ="check_add(this,'.($a+1).')" style="cursor:pointer;" /></td></tr>';
                        }
                        ...
                        if($j%3==2){
                          ...
                          if(strpos($process_text1[$j],"::") === false){
                            if($process_text1[$j] == "::"){
                              ...
                              echo '<tr class="check'.($i-1).'"><td class="tdSolid" colspan="2"><textarea name="work_text'.($i-1).'[]" onkeyup="xSize(this);">'.$process_text1[$j].'</textarea><img src="'.$misc.'img/dashboard/btn/icon_plus.svg" style="cursor:pointer;" class="tdDivisionBtn" onclick="tdDivision(this,'.($basicFormIdx-1).');" /></td>';
                            }
                          }else{
                            $text = explode('::',$process_text1[$j]);
                            echo '<tr class="check'.($i-1).'"><td class="tdSolid"><textarea name="work_text'.($i-1).'[]" onkeyup="xSize(this);">'.$text[0].'</textarea></td>';
                          ...
                          echo '<td class="tdSolid"><textarea name="work_text'.($i-1).'[]" onkeyup="xSize(this);">'.$process_text1[$j].'</textarea></td>
                                  ...
                                  <td align="center;" class ="adddelbtn"><img src="'.$misc.'img/dashboard/btn/icon_minus.svg" style="cursor:pointer;" onclick ="check_del(this)" /></td></tr>';
                        }
                    }
                  }
              }
...
<div align="center"><input type="image" src="<?php echo $misc; ?>img/dashboard/btn/btn_adjust.png" width="70" height="35" style="cursor:pointer" onClick="javascript:chkForm();return false;" /></div>
</form>
</div>
<script>
  textareaSize();
  ...
  function check_add(btn,tableNum) {
    ...
    // }else{
    //   $('#work_text_table'+tableNum).find($(".check0")).before('<tr class="'+trClass+'"><td class="tdSolid" colspan="2"><textarea name="work_text'+trNum+'[]" onkeyup="xSize(this);" style="width:80%;"></textarea><img src="<?php echo $misc;?>img/dashboard/btn/icon_plus.svg" style="cursor:pointer;" class="tdDivisionBtn" onclick="tdDivision(this,'+trNum+');"/></td><td class="tdSolid"> \
    //   <div style="margin:0 auto; text-align: -webkit-center;font-weight:normal;">\
    //   <input type="hidden" name="work_text'+trNum+'[]" value="normal">\
    //   <input type="radio" name="produce'+tableNum+'_'+normalCheck_idx+'" value="normal" onclick="normalCheck(this);" checked="checked">정상\
    //   <input type="radio" name="produce'+tableNum+'_'+normalCheck_idx+'" value="abnormal" onclick="normalCheck(this);">비정상\
  ...
  function tdMerge(btn,num) {
    ...
    td.append('<img src="<?php echo $misc;?>img/dashboard/btn/icon_plus.svg" style="cursor:pointer;" class="tdDivisionBtn" onclick="tdDivision(this,'+num+');"/>');
  }
  ...
  function chkForm(){
    ...
    document.forms['cform'].action="<?php echo site_url(); ?>/tech/tech_board/product_check_list_update_action";
