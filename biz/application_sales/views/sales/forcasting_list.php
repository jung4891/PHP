...
<tbody height="100%">
	...
	<tr id="search_tr" style="<?php if($search_mode == 'simple'){echo 'display:none';} ?>" onkeydown="if(event.keyCode==13) return GoSearch('detail');">
		<td align="left" valign="top">
			<table id="filter_table" class="search_title">
				<colgroup>
					<col width="110px">
					<col width="200px">
					<col width="110px">
				...
				<div style="float:left;white-space:nowrap">
					...
					<tr>
						<td>매출처</td>
						<td>
							<input type="text" id="filter12" class="input-common filtercolumn" value='<?php if(isset($filter)&&$search_mode=='detail'){echo $filter[11];} ?>' />
						</td>
						<td>영업부서</td>
						<td>
					...
					<?php } else {?>
						<input type="hidden" id="filter10" class="input3 filtercolumn" value="" />
					<?php } ?>
						<td>정보통신공사업</td>
						<td colspan="2">
							<select id="filter11" class="select-common select-style1 filtercolumn">
								<option value="">선택</option>
								<option value="Y" <?php if(isset($filter)&&$search_mode=='detail' && $filter[10] == 'Y'){echo "selected";} ?>>신청</option>
								<option value="N" <?php if(isset($filter)&&$search_mode=='detail' && $filter[10] == 'N'){echo "selected";} ?>>미신청</option>
							</select>
						</td>
					</tr>
					<tr>
						...
						<td colspan="3">
							...
							~
							<input type="text" id="filter13" class="input-common filtercolumn datepicker" value='<?php if(isset($filter)&&$search_mode=='detail'){echo $filter[12];} ?>' autocomplete="off"/>
						</td>
					</tr>
				</div>
			</table>
		...
		<td valign="bottom">
			<?php if($mode =="forcasting"){ ?>
			<div>
				<a style="float:right;" href="<?php echo site_url();?>/sales/forcasting/forcasting_input" title="작성하기">
					<input type="button" class="btn-common btn-color2" value="작성">
				</a>
			</div>
		<?php } ?>
		</td>
	...
	<tr id="search_tr2" style="<?php if($search_mode == 'detail'){echo 'display:none';} ?>" onkeydown="if(event.keyCode==13) return GoSearch('simple');">
		<td align="left" valign="top">
			<table width="100%" id="filter_table2">
				<td>
					<div style="float:left;white-space:nowrap">
						<span class="search_title">판매종류</span>
						...
						<?php if($mode =="forcasting"){ ?>
						<span class="search_title">진척단계<span>
						...
						<?php } ?>
						<span class="search_title">정보통신공사업<span>
						...
						<input type="text" id="filter2_4" class="input-common filtercolumn2" value="<?php if(isset($filter)&&$search_mode=='simple'){echo $filter[3];} ?>" style="margin-left:10px;" placeholder="검색하세요.">
	...
	<tr height="35%" id="content_tr">
				...
				<tr>
					<td id="tablePlus">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<colgroup>
								<col width="2.625%" />  <!--번    호 3%-->
								<col width="3.625%" />	<!--종    류-->
								<col width="10.625%" />	<!--고 객 사-->
								<col width="11.625%" />	<!--프로젝트-->
								<col width="4.625%" />	<!--매 출 처-->
								<col width="4.625%" />	<!--제 조 사-->
								<col width="8.625%" />	<!--제 품 명-->
								<col width="5.625%" />	<!--예 상 월-->
								<col width="5.625%" />	<!--진척단계-->
								<col width="6.625%" />	<!--매출금액 7%-->
								<col width="6.625%" />	<!--매입금액 7%-->
								<col width="6.625%" />	<!--마진금액 7%-->
								<col width="4.625%" />	<!--마 진 율-->
								<col width="6.625%" />	<!--업    체-->
								<col width="5.625%" />	<!--담당부서-->
								<col width="5.625%" />	<!--담 당 자-->
	  					</colgroup>
							...
							<?php
				              ...
				              $strType = "조달";
										...
										if($item['progress_step'] == "001") {
											...
											$strStep = "수금완료(100%)";
										}else if($item['progress_step'] == "000") {
											$strStep = "실주";
										}
							?>
	           <?php if($cnum == $item['company_num'] || $sales_lv >= "1") { ?>
							 <tr onmouseover="this.style.backgroundColor='#f5ffff'" onmouseout="this.style.backgroundColor='#fff'" style="cursor:pointer" onclick="ViewBoard('<?php echo $item['seq'];?>')" onmousedown="copy_div(event, this, '<?php echo $item['seq'];?>');">
						 <?php } else {?>
							 <tr onmouseover="this.style.backgroundColor='#f5ffff'" onmouseout="this.style.backgroundColor='#fff'">
						 <?php } ?>
		...
		<tr height="40%" id="paging_tr">
			<td align="center" valign="top" style="padding-bottom:15px;padding-top:15px;" colspan="2">
  ...
  function excelDownload(id, title) {
    ...
    excel_download_table +='<tr><td style="border-top:1px solid #797c88; height:30px;" bgcolor="#f8f8f9" align="center" class="t_border">제조사</td><td style="border-top:1px solid #797c88; height:30px;" bgcolor="#f8f8f9" align="center" class="t_border">제품명</td></tr>';
        ...
        $cnt = 1;
			...
			?>
      <?php if($cnum == $item['company_num'] || $sales_lv == "3") { ?> //주석처리 되어있던 부분
        excel_download_table +='<tr onmouseover="this.style.backgroundColor='+"#FAFAFA"+'" onmouseout="this.style.backgroundColor='+'#fff'+'" style="cursor:pointer" onclick="ViewBoard(<?php echo $item['seq'];?>)">'; //주석처리 되어있던 부분
      <?php } else {?> //주석처리 되어있던 부분
