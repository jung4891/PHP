...
//		$cur_page = $this->input->get( 'cur_page' );			//	현재 페이지
	...
	function customer_input_action() {
		...
		if($seq == null && $this->input->post('company_part') != "002") {
...
//				'cnumfile_realname' => $filename,
		...
		if($seq == null && $result) {
			echo "<script>alert('정상적으로 처리되었습니다.');location.href='".site_url()."/admin/customer/customer_input2/".$mdata['seq']."'</script>";
		} else if($seq != null && $result) {
			echo "<script>alert('정상적으로 처리되었습니다.');location.href='".site_url()."/admin/customer/customer_view2/".$seq."'</script>";
		} else {
	...
	function customer_input_action2() {
		...
		if($mode == 0){
			...
			$update_array = json_decode($this->input->post('update_array'),true);
			$insert_array = json_decode($this->input->post('insert_array'),true);
			$delete_array = $this->input->post('delete_array');

			if(is_array($update_array)){
				for($i=0; $i<count($update_array['value']);$i++){
					$values=explode(";;;",$update_array['value'][$i]);
					$data = array(
						'seq'           => $values[0],
						'user_branch'   => $values[1],
						'user_duty'     => $values[2],
						'user_name'     => $values[3],
						'user_tel'	    => $values[4],
						'user_email'	=> $values[5],
						...
						'manager'	    => $values[7],
						'bill_flag'	    => $values[8],
						'default_flag'	=> $values[9],
						'user_id' 		=> $this->id,
						'update_date'	=> date("Y-m-d H:i:s")
					);
					$result = $this->STC_Customer->customer_staff_update($data);
				}
			}

			if(is_array($insert_array)){
				for($i=0; $i<count($insert_array['value']);$i++){
					$values=explode(";;;",$insert_array['value'][$i]);
					$data = array(
						'customer_seq'  => $seq,
						'user_branch'   => $values[0],
						'user_duty'     => $values[1],
						'user_name'     => $values[2],
						'user_tel'	    => $values[3],
						'user_email'	=> $values[4],
						...
						'manager'	    => $values[6],
						'bill_flag'	    => $values[7],
						'default_flag'	=> $values[8],
						'user_id' 		=> $this->id,
						'insert_date'	=> date("Y-m-d H:i:s")
					);
					$result = $this->STC_Customer->customer_staff_insert($data);
				}
			}

			if($delete_array != ''){
				$result = $this->STC_Customer->customer_staff_delete($delete_array);
			}
		}

		if($mode == 0 && $result) {
			echo "<script>alert('정상적으로 처리되었습니다.');location.href='".site_url()."/admin/customer/customer_input6/".$seq."'</script>";
		} else if($mode == 1 && $result) {
			echo "<script>alert('정상적으로 처리되었습니다.');location.href='".site_url()."/admin/customer/customer_view6/".$seq."'</script>";
		} else {
			echo "<script>alert('정상적으로 처리되지 못했습니다. 다시 입력해 주세요.');history.go(-1);</script>";
		}
	}
	...
	function customer_input_action3() {
		...
		$scheck = $this->input->post('scheck');
		$checksum = $this->input->post('checksum');

		if($checksum == "1") {
			$getcheck = implode(",", $scheck);
		} else {
			$getcheck = "";
		}
		...
		if($mode == 0 && $result) {
			echo "<script>alert('정상적으로 처리되었습니다.');location.href='".site_url()."/admin/customer/customer_input4/".$seq."'</script>";
		} else if($mode == 1 && $result) {
			echo "<script>alert('정상적으로 처리되었습니다.');location.href='".site_url()."/admin/customer/customer_view4/".$seq."'</script>";
		} else {
			echo "<script>alert('정상적으로 처리되지 못했습니다. 다시 입력해 주세요.');history.go(-1);</script>";
		}
	}
	...
	function customer_input_action4() {
		...
		if($mode == 0 && $result) {
			echo "<script>alert('정상적으로 처리되었습니다.');location.href='".site_url()."/admin/customer/customer_input5/".$seq."'</script>";
		} else if($mode == 1 && $result) {
			echo "<script>alert('정상적으로 처리되었습니다.');location.href='".site_url()."/admin/customer/customer_view5/".$seq."'</script>";
		} else {
			echo "<script>alert('정상적으로 처리되지 못했습니다. 다시 입력해 주세요.');history.go(-1);</script>";
		}
	}
...
//					'gfile_realname' =>  $filename5,
	...
	function customer_input_action6() {
		...
		if($mode == 0 && $result) {
			echo "<script>alert('정상적으로 처리되었습니다.');location.href='".site_url()."/admin/customer/customer_list'</script>";
		} else if($mode == 1 && $result) {
			echo "<script>alert('정상적으로 처리되었습니다.');location.href='".site_url()."/admin/customer/customer_list'</script>";
		} else {
			echo "<script>alert('정상적으로 처리되지 못했습니다. 다시 입력해 주세요.');history.go(-1);</script>";
	...
	function customer_filedel($seq, $filelcname) {
		...
		if (!isset($fdata['bfile_changename'])) {
			...
			alert('파일이 정상적으로 삭제되었습니다.', site_url().'/admin/customer/customer_view5/'.$seq);
		}
	}
	...
	function customer_input() {
		...
		// $this->load->Model(array('STC_Customer', 'STC_Common'));
		$this->load->view('admin/customer_input');
	}
	...
	function customer_input2($seq) {
		...
		$data['seq'] = $seq;
		// $this->load->Model(array('STC_Customer', 'STC_Common'));
		$this->load->view('admin/customer_input2' , $data);
	}
	...
	function customer_input3($seq) {
		...
		$no_page_list = 2;										//	한페이지에 나타나는 목록 개수
...
//		$data['count'] = $this->STC_Customer->sourcing_list_count($search_keyword, $search1, $search2)->ucount;
		...
		$this->load->view('admin/customer_input3' , $data);
	}
	...
	function customer_input4($seq) {
		...
		$data['seq'] = $seq;
		// $this->load->Model(array('STC_Customer', 'STC_Common'));
		$this->load->view('admin/customer_input4' , $data);
	}
	...
	function customer_input5($seq) {
		...
		$data['seq'] = $seq;
		// $this->load->Model(array('STC_Customer', 'STC_Common'));
		$this->load->view('admin/customer_input5' , $data);
	}
	...
	function customer_input6($seq) {
		...
		$data['seq'] = $seq;
		// $this->load->Model(array('STC_Customer', 'STC_Common'));
		$this->load->view('admin/customer_input6' , $data);
	}
	...
	function customer_view() {
		...
		if( $this->id === null ) {
			if($this->cooperative_id <> null){
				if ($_SESSION['timeout'] + 10*60 < time()) {
					...
					echo "<script>location.href='".site_url()."/admin/customer/customer_view2/".$seq."'</script>";
				}
			}else{
				redirect( 'account' );
			}
		...
		$this->load->view('admin/customer_modify', $data );
	}
...
//		echo "seq::".$seq."<br>";
		...
		if($strResult == false) {
			echo "<script>alert('입력된 내용이 없습니다. 입력페이지로 이동합니다.');location.href='".site_url()."/admin/customer/customer_input2/".$seq."'</script>";
			exit;
		}
		...
		$this->load->view('admin/customer_modify2', $data );
	}
	...
	function customer_view3($seq) {
		...
		if($strResult == false) {
			echo "<script>alert('입력된 내용이 없습니다. 입력페이지로 이동합니다.');location.href='".site_url()."/admin/customer/customer_input3/".$seq."'</script>";
			exit;
		}
...
//		$data['count'] = $this->STC_Customer->sourcing_list_count($search_keyword, $search1, $search2)->ucount;
		...
		$this->load->view('admin/customer_modify3', $data );
	}
	...
	function customer_view4($seq) {
		...
		// 거래처 서비스(상품)이 없을경우 입력페이지로 이동
		if($strSupply == false) {
			echo "<script>alert('입력된 내용이 없습니다. 입력페이지로 이동합니다.');location.href='".site_url()."/admin/customer/customer_input4/".$seq."'</script>";
			exit;
		}
		...
		$this->load->view('admin/customer_modify4', $data );
	}
	...
	function customer_view5($seq) {
		...
		if($strResult == false) {
			echo "<script>alert('입력된 내용이 없습니다. 입력페이지로 이동합니다.');location.href='".site_url()."/admin/customer/customer_input5/".$seq."'</script>";
			exit;
		}
		...
		$this->load->view('admin/customer_modify5', $data );
	}
	...
	function customer_view6($seq) {
		...
		// 거래처 재무정보가 없을경우 입력페이지로 이동
		if($strResult == false) {
			echo "<script>alert('입력된 내용이 없습니다. 입력페이지로 이동합니다.');location.href='".site_url()."/admin/customer/customer_input6/".$seq."'</script>";
			exit;
		}
		...
		$this->load->view('admin/customer_modify6', $data );
	}

	// 거래처 삭제
	function customer_delete_action($seq) {
		if( $this->id === null ) {
			redirect( 'account' );
		}
...
//		$seq = $this->input->post( 'seq' );
		...
		if ($tdata) {
			echo "<script>alert('삭제완료 되었습니다.');location.href='".site_url()."/admin/customer/customer_list'</script>";
		} else {
...
//		$cur_page = $this->input->get( 'cur_page' );			//	현재 페이지
		...
		$this->load->view('admin/sourcing_list', $data );
	}
	...
	function sourcing_input_action() {
		...
		if($result) {
			echo "<script>alert('정상적으로 처리되었습니다.');location.href='".site_url()."/admin/customer/sourcing_list'</script>";
		} else {
			echo "<script>alert('정상적으로 처리되지 못했습니다. 다시 입력해 주세요.');history.go(-1);</script>";
		}
	...
	function sourcing_input() {
		...
		// $this->load->Model(array('STC_Customer', 'STC_Common'));
		$data['category'] = $this->STC_Common->get_category();
		$this->load->view('admin/sourcing_input', $data );
	}
...
//		$user_id = $this->idx;
		...
		$this->load->view('admin/sourcing_modify', $data );
	}
	...
	function sourcing_delete_action() {
		...
		if ($tdata) {
			echo "<script>alert('삭제완료 되었습니다.');location.href='".site_url()."/admin/customer/sourcing_list'</script>";
		} else {
			alert("정상적으로 처리되지 못했습니다.\n다시 시도해 주세요.");
		}
	}
