...
class Maintain extends CI_Controller {
	...
	function maintain_list() {
		...
		//amount 금액조회
		$amount_list_data = $this->STC_Maintain->maintain_list($type, 'sum', $search_keyword,0, 0, $this->cnum);
		$data['amount_list_val'] = $amount_list_data['data'];
		$user_list_data = $this->STC_Maintain->maintain_list($type,'list',$search_keyword, ( $cur_page - 1 ) * $no_page_list, $no_page_list, $this->cnum);
		// $data['count'] = $this->STC_Maintain->maintain_list_count($type,$search_keyword,$this->cnum)->ucount;
		$list_count = $this->STC_Maintain->maintain_list($type, 'count', $search_keyword,0, 0, $this->cnum);
		// var_dump($list_count);
	...
	function excel_data(){
		...
		$result = $this->STC_Maintain->maintain_excel_download($type,$search_keyword,$this->cnum);
