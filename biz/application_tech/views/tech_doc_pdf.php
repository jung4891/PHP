...
$email = $this->phpsession->get( 'email', 'stc' );
?>
<!DOCTYPE html
...
<body>
    <table id="cover" class="borderNone" border=0.5 cellpadding=0 cellspacing=0 width=730 height=1100
        style='page-break-after:always;border-collapse:collapse;table-layout:fixed;width:100%;<?php if($view_val['cover'] != 'basic'){echo 'background-image:url("'.$misc.'img/cover/'.$cover[0]['cover_name'].'");background-size:100% 100%;';}?>'>
        ...
        <tr class="borderNone" height=100>
            ...
            <td colspan=4 class="borderNone" style="font-size:30px;font-weight:bold;text-align:center;">
                [<?php echo $view_val['customer'];?> 기술지원 보고서 ]</td>
            <?php }else{ ?>
            <td colspan=4 class="borderNone" style="font-size:30px;font-weight:bold;text-align:center;">
                [<?php echo $view_val['customer'];?> 정기점검 보고서 ]</td>
            <?php } ?>
        </tr>
    ...
    <?php if($view_val['work_name'] == "정기점검2"){ ?>
    <table id="check_table" border=0.5 align="center" cellpadding=0 cellspacing=0
        style="border-collapse:collapse;table-layout:fixed;width:100%;page-break-after: always;">
        <tr>
    ...
    <table border=0.5 cellpadding=0 cellspacing=0 width=730
        ...
        <tr height=22 style='mso-height-source:userset;height:16.5pt'>
            <td colspan=25 rowspan=2 height=44 class=xl7423169
                style='border-right:1.0pt solid black;border-bottom:1.0pt solid black;height:33.0pt'>기술지원보고서</td>
        </tr>
        <tr height=22 style='mso-height-source:userset;height:16.5pt'>
        </tr>
        ...
        <tr height=37 style='mso-height-source:userset;'>
            ...
            <td colspan=21 class=xl6723169 style='border-left:none'>제품명 / host / 버전 / 서버 / 라이선스<?php if((trim($view_val['customer']) == "SKB(유통망)" || trim($view_val['customer']) == "SKB(과금망)") && strpos($view_val['produce'],',')=== false) { echo " / serial";} ?></td>
        ...
        <tr height=37 style='mso-height-source:userset;'>
            <td colspan=21 height=37 class=xl6723169
				...
				for($i=1; $i<=count($produce); $i++){
                    echo $produce[($i-1)].' / '.$host[($i-1)].' / '.$version[($i-1)].' / '.$hardware[($i-1)].' / '.$license[($i-1)];
    ...
    <table border=0.5 align="center" cellpadding=0 cellspacing=0
        style="border-collapse:collapse;table-layout:fixed;width:100%;">
        <tr style='mso-height-source:userset;height:16.5pt'>
            <td colspan=25 rowspan=2 height=44 class=xl7423169
                style='border-right:1.0pt solid black;border-bottom:1.0pt solid black;height:33.0pt'>정기점검보고서</td>
        </tr>
    ...
    <table id="work_text_table<?php echo ($a+1) ;?>" class="work_text_table" border=0.5 cellpadding=0 cellspacing=0
        style='border-collapse:collapse;table-layout:fixed;width:99.9%;border-bottom:1px;<?php if($a != (count($produce)-1)){echo "page-break-after:always;";} ?>'>
        <tr>
            <th width="126" height=22 style='mso-height-source:userset;'>
                <input value="점검항목" readonly onfocus="javascrpt:blur();" style="cursor:default; font-weight:bold; border:none; text-align:center; width:95%;height:95%;background:transparent;" />
            </th>
            <th colspan="2" height=22 style='mso-height-source:userset;'>
                <input value="점검내용" readonly onfocus="javascrpt:blur();" style="cursor:default; font-weight:bold; border:none; text-align:center; width:95%;height:95%;background: transparent;" />
            </th>
            <th height=22 style='mso-height-source:userset;'>
                <input value="점검결과" readonly onfocus="javascrpt:blur();" style="cursor:default; font-weight:bold; border:none; text-align:center; width:95%; height:95%;background: transparent;" />
            </th>
            <th height=22 style='mso-height-source:userset;'>
                <input value="특이사항" readonly onfocus="javascrpt:blur();" style="cursor:default; font-weight:bold; border:none; text-align:center; width:95%; height:95%;background: transparent;" />
            </th>
        </tr>
        <?php
...
for($j=1; $j<count($process_text1); $j++){
    ...
    <?php }?>
    <table border=0.5 cellpadding=0 cellspacing=0 style='border-collapse:collapse;table-layout:fixed;width:99.9%;'>
        <tr height=55 style='mso-height-source:userset;min-height:41.25pt'>
            <td colspan=4 width="118" height=55 class=xl8023169 style='min-height:41.25pt'>지원의견</td>
            <td colspan=21 class=xl8223169 width=619 style='border-right:.5pt solid black;width:467pt'>
                <?php echo $view_val['comment'];?></td>
        </tr>
    </table>
    <?php } ?>
    <?php if($view_val['work_name'] != "정기점검2"){ ?>
    ...
    <tr height=55 style='mso-height-source:userset;min-height:41.25pt'>
        ...
        <td colspan=21 class=xl8223169 width=619
            style='border-right:.5pt solid black;width:467pt;padding-left:10px;padding-right:10px;'>
