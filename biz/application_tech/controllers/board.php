...
//              $cur_page = $this->input->get( 'cur_page' );                    //      현재 페이지
                ...
                $this->load->view( 'network_map_list', $data );
        }
...
//		$cur_page = $this->input->get( 'cur_page' );			//	현재 페이지
	...
	function faq_input_action() {
		...
		if($this->input->post('subject')==null){
			echo "<script>alert('비정상적 접근입니다.');location.href='".site_url()."/board/faq_input'</script>";
		}
...
//		$user_id = $this->idx;
	...
	function faq_delete_action() {
        ...
        function network_map_input_action() {
                ...
                if($result) {
                        echo "<script>alert('정상적으로 처리되었습니다.');location.href='".site_url()."/board/network_map_list'</script>";
                } else {
                        echo "<script>alert('정상적으로 처리되지 못했습니다. 다시 입력해 주세요.');history.go(-1);</script>";
                }
 ...
 //릴리즈노트 입력/수정시 ckeditor 이미지 본문삽입 처리
		...
		$data['customer'] = $this->STC_tech_doc->get_customer();
                $this->load->view( 'network_map_input', $data );
        }
...
//              $user_id = $this->idx;
                ...
                if($mode == "view") {
                        $this->load->view( 'network_map_view', $data );
                } else {
                        $this->load->view( 'network_map_modify', $data );
                }
        }
        ...
        function network_map_delete_action() {
                ...
                if ($tdata) {
                        echo "<script>alert('삭제완료 되었습니다.');location.href='".site_url()."/board/network_map_list'</script>";
                } else {
                        alert("정상적으로 처리되지 못했습니다.\n다시 시도해 주세요.");
                }
        }
...
//		$cur_page = $this->input->get( 'cur_page' );			//	현재 페이지
		...
		$data['start_page'] = $start_page;
		$data['end_page'] = $end_page;
			$this->load->view( 'release_note_list', $data );
	}
	...
	function release_note_filedel($seq, $filelcname) {
		...
		$this->load->Model('STC_Board');
			...
			alert('파일이 정상적으로 삭제되었습니다.', site_url().'/board/release_note_view?seq='.$seq.'&mode=modify');
		}
	}
...
*/
		...
		if($result) {
			echo "<script>alert('정상적으로 처리되었습니다.');location.href='".site_url()."/board/release_note_list'</script>";
		} else {
			echo "<script>alert('정상적으로 처리되지 못했습니다. 다시 입력해 주세요.');history.go(-1);</script>";
		}
	...
	function release_note_input() {
		...
		$this->load->Model(array('STC_Board', 'STC_Common'));
		$data['category'] = $this->STC_Common->get_category();
		$this->load->view( 'release_note_input', $data );
	}
...
//		exit;
		...
		if($mode == "view") {
			$this->load->view( 'release_note_view', $data );
		} else {
			$this->load->view( 'release_note_modify', $data );
		}
	}
	...
	function release_note_delete_action() {
		...
		if ($tdata) {
			echo "<script>alert('삭제완료 되었습니다.');location.href='".site_url()."/board/release_note_list'</script>";
		} else {
			alert("정상적으로 처리되지 못했습니다.\n다시 시도해 주세요.");
		}
	}
