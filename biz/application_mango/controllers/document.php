...
class Document extends CI_Controller {
	...
	function __construct() {
		...
		$this->load->Model(array('STC_Document', 'STC_Common'));
	...
	function document_list() {
		...
		$user_list_data = $this->STC_Document->document_list($search_keyword, $search1, ( $cur_page - 1 ) * $no_page_list, $no_page_list);
		$data['count'] = $this->STC_Document->document_list_count($search_keyword, $search1)->ucount;
	...
	function document_view() {
		...
		$data['view_val'] = $this->STC_Document->document_view($seq);
		$data['seq'] = $seq;
		...
		if($mode == 'view') {
			$this->load->view( 'document/document_view', $data );
		} else {
			$this->load->view( 'document/document_modify', $data );
		}
	}
	...
	function document_input() {
		...
		$this->load->view( 'document/document_input');
	}

	//공지사항 입력/수정 처리
	function document_input_action() {
		if( $this->id === null ) {
			redirect( 'account' );
		}
		...
		if($file_count > 0){
			for($i=0; $i<$file_count; $i++){
				...
				//     exit;
				// }
				$upload_dir = "/var/www/html/stc/misc/upload_mango/document";
				// $upload_dir = "c:/xampp/htdocs/biz/misc/upload_mango/document";
			...
			$data = array(
				'document_name'   => addslashes($this->input->post('subject')),
			...
			if ($seq == null) {
				...
				$result = $this->STC_Document->document_insert($data, $mode = 0);
			} else {
				$result = $this->STC_Document->document_insert($data, $mode = 1, $seq);
			}
	...
	// 공지사항 삭제
	function document_delete_action() {
		if( $this->id === null ) {
			redirect( 'account' );
		}

		$this->load->helper('alert');
		// $this->load->Model( 'STC_Document' );
		$seq = $this->input->get( 'seq' );

		if ($seq != null) {
			$tdata = $this->STC_Document->document_delete($seq);
		}

		if ($tdata) {
			echo "<script>alert('삭제완료 되었습니다.');location.href='".site_url()."/document/document_list'</script>";
		} else {
