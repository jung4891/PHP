...
//KI1 20210125 주석처리함 - ajax대신 submit을 사용
//일정 등록
...
//   var forcasting_seq = $('#customer').val();
//   //KI2@@@@@
...
//KI1 20210125 주석처리함 - 참석자 형태를 검색 참석자 형태로 변경하면서 삭제한 함수
...
//KI1 20210125 참석자 추가 형태를 검색 참석자 추가 형태로 변경
// 참석자 추가
function addUser(idVal){
  ...
  if(idVal == 'searchChosenBtn'){
    ...
    var participantAdduser = participantNameArr.join(",");
    $('#participant').val('');
    $('#participant').val(participantAdduser);
    // $('#engineer').val(noneOverlapArr);
    $('#addUserpopup').bPopup().close(); //모달 닫기
  }
}
...
//KI1 20210125 주석처리함
  // $('#customer').chosen({width:"270px"}).change(function(){
  //   if($(this).val()=='직접입력'){
  //     $('#insertDirect').show();
  //   } else {
  //     $('#insertDirect').hide();
...
//KI1 20210125 입력칸이 빈칸일 때 일정 등록이 불가하도록 조건 제한
function checkAll() {
  if($('#startDay').val() == ''){
    alert('시작날짜를 입력해주세요.');
    $('#startDay').focus();
