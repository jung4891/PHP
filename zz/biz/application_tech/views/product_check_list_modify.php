...
<div style="margin:10px 0 10px 0 ;"><span class="title">제품명</span><input type="text" name="product_name" id="product_name" class="input2" value="<?php echo $view_val[0]['product_name'] ;?>" ></div>
<?php
            ...
            $process_text1 = explode('#*',$process_text[$i]); //점검 내용 별로 나누기
              if($i <> 1){ //기타 특이사항을 제외한 나머지
                  ...
                  for($j=1; $j<count($process_text1); $j++){
                    if($j==1){
                        echo '<tr class="check'.($i-1).'"><td class="tdSolid" rowspan="'.floor((count($process_text1)-1)/3).'"><input name="work_text'.($i-1).'[]" value="'.$process_text1[$j].'" style="width:70%;word-wrap:break-word;"></input><img src="'.$misc.'img/btn_del0.jpg" style="cursor:pointer;" class="adddelbtn" onclick="trRemove(this,'.($a+1).');" /></td>'; //cpu, 메모리 
...
<div><input type="image" src="<?php echo $misc; ?>img/btn_adjust.jpg" width="64" height="31" style="cursor:pointer" onClick="javascript:chkForm();return false;" /></div>
</form>
</div>
<script>
  textareaSize();
  ...
  function chkForm(){
    ...
    document.forms['cform'].action="<?php echo site_url(); ?>/tech_board/product_check_list_update_action";
