...
//		$cur_page = $this->input->get( 'cur_page' );			//	현재 페이지
		...
		$this->load->view( 'companynum_list', $data );
	}
	...
	function companynum_input_action() {
		...
		if($result) {
			echo "<script>alert('정상적으로 처리되었습니다.');location.href='".site_url()."/company/companynum_list'</script>";
		} else {
	...
	function companynum_input() {
		...
		$this->load->Model(array('STC_Company', 'STC_Common'));
		$data['category'] = $this->STC_Common->get_category();
		$this->load->view( 'companynum_input', $data );
	}
...
//		$user_id = $this->idx;
		...
		$this->load->view( 'companynum_modify', $data );
	}
	...
	function companynum_delete_action() {
		...
		if ($tdata) {
			echo "<script>alert('삭제완료 되었습니다.');location.href='".site_url()."/company/companynum_list'</script>";
		} else {
...
//		$cur_page = $this->input->get( 'cur_page' );			//	현재 페이지
	...
	function product_input_action() {
		...
		if($result) {
			echo "<script>alert('정상적으로 처리되었습니다.');location.href='".site_url()."/company/product_list'</script>";
		} else {
			echo "<script>alert('정상적으로 처리되지 못했습니다. 다시 입력해 주세요.');history.go(-1);</script>";
		}
	...
	function product_input() {
		...
		$this->load->Model(array('STC_Company', 'STC_Common'));
		$data['category'] = $this->STC_Common->get_category();
		$this->load->view( 'product_input', $data );
	}
...
//		$user_id = $this->idx;
		...
		$this->load->view( 'product_modify', $data );
	}
	...
	function product_delete_action() {
		...
		if ($tdata) {
			echo "<script>alert('삭제완료 되었습니다.');location.href='".site_url()."/company/product_list'</script>";
		} else {
			alert("정상적으로 처리되지 못했습니다.\n다시 시도해 주세요.");
		}
	}
