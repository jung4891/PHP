...
//		$this->user_id = $this->phpsession->get( 'id', 'stc' );
	...
	function customer_insert2( $data, $mode = 0 , $seq = 0) {
		if( $mode == 0 ) {
			...
			for($i=1;$i<count($person_array);$i++)
			{
				...
				$sql = "insert into sales_customer_staff (customer_seq,user_duty,user_name,user_tel,user_email,user_work,bill_flag,default_flag,user_id,insert_date,update_date) values(?,?,?,?,?,?,?,?,?,now(),now())";
				$result = $this->db->query($sql, array($data['seq'],$person_list[0],$person_list[1],$person_list[2],$person_list[3],$person_list[4],$person_list[5],$person_list[6],$data['user_id']));
			}
	...
	//	로그인한 사람의 사업자번호 받아서 seq가져오기
	function customer_getseq( $cnum = 0 ) {
		$sql = "select seq from sales_customer_basic where cnum = ?";
		$query = $this->db->query( $sql, $cnum );
	...
	//	로그인한 사람의 seq 체크
	function customer_view_check( $seq = 0 ) {
