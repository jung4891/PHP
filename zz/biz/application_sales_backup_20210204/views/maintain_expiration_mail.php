...
if(count($expiration30)== 0 && count($expiration15) == 0){
    ...
    <th style='height:22.5pt;width:60pt;text-align:center;vertical-align:middle;border:.5pt solid windowtext;background:#D9D9D9;'>사업명/연계프로젝트명</th>
    ...
    foreach($expiration30 as $exp30){
     ...
     <td style='text-align:center;vertical-align:middle;border:.5pt solid windowtext;'>";
    if($exp30['parent_project_name'] == null ){ 
        $txt .= $exp30['project_name'];
    }else{
        $txt .= $exp30['parent_project_name']." / {$exp30['project_name']}"; 
    }
     $txt .=".</td>
     <td style='text-align:center;vertical-align:middle;border:.5pt solid windowtext;'>{$exp30['exception_saledate2']}~";
    if($exp30['parent_exception_salesdate'] == null ){
        $txt .= $exp30['exception_saledate3'];
    }else{
        $txt .= $exp30['parent_exception_salesdate'];
    }
     $txt .= "</td>
    ...
    foreach($expiration15 as $exp15){
     ...
     <td style='text-align:center;vertical-align:middle;border:.5pt solid windowtext;'>";
     if($exp15['parent_project_name'] == null ){ 
        $txt .= $exp15['project_name'];
    }else{
        $txt .= $exp15['parent_project_name']." / {$exp15['project_name']}"; 
    }
     $txt .= "</td>
     <td style='text-align:center;vertical-align:middle;border:.5pt solid windowtext;'>{$exp15['exception_saledate2']}~";
    if($exp15['parent_exception_salesdate'] == null ){
        $txt .= $exp15['exception_saledate3'];
    }else{
        $txt .= $exp15['parent_exception_salesdate'];
    }
     $txt .= "</td>
    ...
    foreach($after_expiration as $after_exp){
        ...
        <td style='text-align:center;vertical-align:middle;border:.5pt solid windowtext;'>";
        if($after_exp['parent_project_name'] == null ){ 
            $txt .= $after_exp['project_name'];
        }else{
            $txt .= $after_exp['parent_project_name']." / {$after_exp['project_name']}"; 
        }
        $txt .= "</td>
        <td style='text-align:center;vertical-align:middle;border:.5pt solid windowtext;'>{$after_exp['exception_saledate2']}~";
        if($after_exp['parent_exception_salesdate'] == null ){
            $txt .= $after_exp['exception_saledate3'];
        }else{
            $txt .= $after_exp['parent_exception_salesdate'];
        }
        $txt .= "</td>
