			 ...
			 <tr>
				<td align="right">
					<span style="margin-top:30px;">
						<img src="<?php echo $misc;?>img/btn_cancel.jpg" width="64" height="31" style="cursor:pointer;" onclick="location.href='<?php echo site_url();?>/maintain/integration_maintain_list'"/>
...
<script>
	//select box 검색 기능
	$("#check_product_company0").select2();
    ...
    function productAdd(){
		...
		html += '<select name="product_company" id="product_company'+$("#row_max_index2").val()+'" class="input2" onchange="productSearch('+$("#row_max_index2").val()+');product_type_default('+$("#row_max_index2").val()+')"><option value="">제조사</option>';
		<?php foreach($product_company as $pc){?>
			html += "<option value='<?php echo $pc['product_company'];?>'><?php echo $pc['product_company']; ?></option>";
		<?php } ?>
		html += '<td height="40" align="center" bgcolor="f8f8f9" class="t_border" style="font-weight:bold;">하드웨어/소프트웨어</td><td align="left" class="t_border" style="padding-left:10px;" colspan="1">';
	...
	function chkForm(type){
		...
		if (objproduct_seq.length > 0) {
			for (var i = 0; i < objproduct_seq.length; i++) {
				$("#update_product_array").val($("#update_product_array").val() + "||" + objproduct_name[i].value +"~" + objproduct_licence[i].value + "~" + objproduct_serial[i].value + "~" + objproduct_seq[i].value);
			}
		}
		...
		if (objproduct_name.length > objproduct_seq.length) {
			for (i = objproduct_seq.length; i < objproduct_name.length; i++) {
				$("#insert_product_array").val($("#insert_product_array").val() + "||"  + objproduct_name[i].value + "~" + objproduct_licence[i].value + "~" + objproduct_serial[i].value);
			}
		}
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: "<?php echo site_url(); ?>/maintain/integration_maintain_input_action",
			dataType: "json",
			async: false,
			data: {
			...
			success: function(data) {
				if(data){
					alert("ok");
					location.href="<?php echo site_url();?>/maintain/integration_maintain_list";
				}else{
					alert("실패");
				}
			}
