									...
									<td>
										<table width="100%" border="0" cellspacing="0" cellpadding="0" id="main_list">
											<form name="cform" action="<?php echo site_url(); ?>/maintain/maintain_input_action" method="post" onSubmit="javascript:chkForm();return false;">
												...
												$j = 1;
												$i = 0;
												foreach($view_val4 as $item4){
													echo "<tr>
															<td colspan='10' height='2' bgcolor='#797c88'></td>
														</tr>
														<tr>
															<td height='40' align='center' bgcolor='f8f8f9' style='font-weight:bold;'>프로젝트명</td>
															<td colspan=9 align='left' class='t_border' style='padding-left:10px;'>{$item4['project_name']}</td>
														</tr>
														<tr>
															<td colspan='10' height='1' bgcolor='#e8e8e8'></td>
														</tr>";
												foreach ($view_val3 as $item3) {
													if($item4['project_name'] == $item3['project_name']){?>
														<tr id="product_insert_field_1_<?php echo $j; ?>">
															<!-- <td height="40" align="center" bgcolor="f8f8f9" style="font-weight:bold;">프로젝트명</td>
															<td align="left" class="t_border" style="padding-left:10px;"><?php echo $item3['project_name']."(".$item3['exception_saledate'].")"; ?></td> -->
															<td align="center" bgcolor="f8f8f9" style="font-weight:bold;">제조사</td>
...
<script type="text/babel" data-presets="es2015, stage-3">
   ...
   //수주여부 코멘트 삭제
    ...
    function modifyPopup(type){
        ...
        window.open("<?php echo site_url(); ?>/maintain/maintain_modify?seq=<?php echo $view_val['seq'] ;?>&type="+type,"","width = 1200, height = 500, scrollbars=1,resizable=yes");
	}

	// 통합유지보수 리스트
	function integration_maintain(){
		window.open("<?php echo site_url(); ?>/maintain/integration_maintain_list","","width = 1000, height = 500, scrollbars=1,resizable=yes");
	}
