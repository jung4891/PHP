...
<script>
	$(function() {
		$("#main_add").click(function() {
			...
			var html ="<tr class='" + id + " tbl-tr cell-tr border-t'><td class='tbl-title' style='height:70px;'>매입처</td><td class='tbl-cell' style='padding-right:10px;'>";
			...
			html += "</select><br><input name='main_companyname' type='text' class='input-common' id='main_companyname"+$("#row_max_index").val()+"'style='width:195px;'/></td><td class='tbl-title'>담당자</td><td class='tbl-cell'>";
			html += '<select id="select_main_user'+$("#row_max_index").val()+'" class="select-common" onchange="selectMainUser('+$("#row_max_index").val()+')"><option value="">담당자 선택</option></select><br>';
			html += "<input name='main_username' type='text' class='input-common' id='main_username"+$("#row_max_index").val()+"' style='width:195px;'/></td></tr><tr class='" + id + " tbl-tr cell-tr'><td class='tbl-title'>연락처</td><td class='tbl-cell'><input name='main_tel' type='text' class='input-common' id='main_tel"+$("#row_max_index").val()+"' onclick='checkNum(this);' onKeyUp='checkNum(this);' style='width:195px;'/></td><td class='tbl-title'>이메일</td><td class='tbl-cell'><input name='main_email' type='text' class='input-common' id='main_email"+$("#row_max_index").val()+"' style='width:195px;'/><img src='<?php echo $misc; ?>img/btn_del0.jpg' style='cursor:pointer;float:right;' onclick='javascript:main_list_del(" + $("#row_max_index").val() + ");'/></td></tr>";
			html += "<tr class=" + id + "><td colspan='4' height='2' bgcolor='#797c88'></td></tr>";
			$('#main_insert').before(html);
			$("#select_main_company"+$("#row_max_index").val()).select2({width:'100%'});
		...
		$("#product_add").click(function() {
			...
			var html = "";
			html += '<tr id="'+id+'"><td height="40" align="center"><input type="checkbox" name="product_row" class="drop" value="'+row_num+'" /></td>';
			html += '<td height="40" align="center" ><span class="p_num">'+$("#row_max_index2").val()+'</span></td>';
			html += '<td align="left">';
			html += '<select name="product_company" id="product_company'+row_num+'" class="input7" onchange="product_type_default('+row_num+');productSearch('+row_num+');">';
			...
			html += '</select></td>';
			html += '<td align="left">';
			html += '<select name="product_supplier" id="product_supplier'+row_num+'" class="input7">';
      html += '<option value=""></option>';
			...
			html += '</select></td>';
			html += '<td align="left">';
			html += '<select name="product_type" id="product_type'+row_num+'" class="input7" onchange="productSearch('+row_num+');">';
			...
			html += '</select></td>';
			html += '<td align="left">'
			html += '<select name ="product_name" id="product_name'+row_num+'" class="input7" onclick="productSearch('+row_num+');">';
			html += '<option value="" >제품선택</option>';
			html += '</select>';
			html += '</td>';
			html += '<td align="left"><input name="product_licence" type="text" class="input7" id="product_licence'+row_num+'" value="" onkeyup ="commaCheck(this);" /> </td>';
			html += '<td align="left"><input name="product_serial" type="text" class="input7" id="product_serial'+row_num+'" value="" onkeyup ="commaCheck(this);" /> </td>';
			html += '<td align="left">';
			html += '<select name="product_state" id="product_state'+row_num+'" class="input5">';
			...
			html += '</select></td>';
			html += '<td align="left"><input name="product_sales" type="text" class="input7" id="product_sales'+row_num+'" value="0" onclick="numberFormat(this);" onchange="numberFormat(this);product_profit_change('+row_num+',0);" style="text-align:right" /> </td>';
			html += '<td align="left"><input name="product_purchase" type="text" class="input7" id="product_purchase'+row_num+'" value="0" onclick="numberFormat(this);" onchange="numberFormat(this);product_profit_change('+row_num+',0);" style="text-align:right" /> </td>';
			html += '<td align="left"><input name="product_profit" type="text" class="input7" id="product_profit'+row_num+'" value="0" style="text-align:right" readonly /></td>';
			html += '<td align="left"><input name="comment" type="text" class="input7" id="comment'+row_num+'" value="" /></td>';
			html += '<td align="center"><img src="<?php echo $misc; ?>img/btn_del0.jpg" onclick="javascript:product_list_del('+$("#row_max_index2").val()+');" style="cursor:pointer;" /></td></tr>';
  ...
  function p_numbering() {
	...
	function t_forcasting_profit_change() {
		...
		for (i = 0; i < document.getElementsByName("product_sales").length; i++) {
			var forcasting_sales = parseInt(mform.forcasting_sales.value.replace(/,/g, "")) + parseInt(document.getElementsByName("product_sales")[i].value.replace(/,/g, ""));
			var forcasting_purchase = parseInt(mform.forcasting_purchase.value.replace(/,/g, "")) + parseInt(document.getElementsByName("product_purchase")[i].value.replace(/,/g, ""));
			mform.forcasting_sales.value = String(forcasting_sales).replace(/(\d)(?=(?:\d{3})+(?!\d))/g, "$1,");
			mform.forcasting_purchase.value = String(forcasting_purchase).replace(/(\d)(?=(?:\d{3})+(?!\d))/g, "$1,");
		}
...
<script language="javascript">
	...
	var chkForm = function() {
      ...
      $('#progress_step option').removeAttr('disabled');
			...
			if(objproduct_seq.length > 0){
				...
				$("#update_product_array").val($("#update_product_array").val() + "||" + objproduct_name[i].value + "~" +objproduct_supplier[i].value+ "~" +objproduct_licence[i].value + "~" + objproduct_serial[i].value + "~" + objproduct_state[i].value + "~" + objproduct_product_sales[i].value.replace(/,/g, "") + "~" + objproduct_product_purchase[i].value.replace(/,/g, "")+ "~" + objproduct_product_profit[i].value.replace(/,/g, "") + "~" + objproduct_comment[i].value +"~"+ objproduct_seq[i].value);
...
<table width="100%" height="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td width="100%" align="center" valign="top">
			...
			<table width="100%" border="0" style="padding:50px;">
				...
				<tr>
				<td class="title3">
				<?php if($view_val['progress_step'] > '014'){
					echo "수주완료";
				}else if ($view_val['progress_step'] == '000'){
					echo "실주";
				}else{
					echo "포캐스팅";
				}?>
            ...
            <input type="button" class="btn-common btn-color1" width="64" height="31" style="cursor:pointer;margin-right:10px;" onclick="popup_close();" value="취소" />
                  ...
                  <td class="tbl-title">담당자
                    ...
                    <input name="customer_username" type="text" class="input-common" id="customer_username" value="<?php echo $view_val['customer_username']; ?>" />
								...
								<tr class="tbl-tr cell-tr">
									...
									<td class="tbl-cell">
                    <input type="text" name="customer_tel" id="customer_tel" class="input-common" value="<?php echo $view_val['customer_tel']; ?>" onclick="checkNum(this);" onKeyUp="checkNum(this);"/>
                  </td>
								</tr>
								<tr class="tbl-tr cell-tr">
                    ...
                    <input name="project_name" type="text" class="input-common" id="project_name" value="<?php echo $view_val['project_name']; ?>" />
								...
								<tr class="tbl-tr cell-tr">
									...
									<td colspan="3" class="tbl-cell">
										<select name="type" id="type" class="select-common" onclick="project_type(this.value);" style="width:120px;">
								...
								<tr class="tbl-tr cell-tr">
									...
									<td class="tbl-cell">
										...
										<select name="progress_step" id="progress_step" class="select-common" onchange="mistake_order(this.value);" style="width:120px;">
											...
											<option value="000" <?php if ($view_val['progress_step'] == "000") {
												echo "selected";
											} ?>>실주</option>
										</select></td>

										<td class="mistake_order" height="40" align="center" bgcolor="F4F4F4" style="font-weight:bold;<?php if($view_val['progress_step'] != "000"){echo "display:none";}?>;">실주사유</td>
										<td class="mistake_order" align="left" style="padding-left:10px;<?php if($view_val['progress_step'] != "000"){echo "display:none";}?>;">
											<input type="text" id="mistake_order_reason" name="mistake_order_reason" class="input-common" value="<?php echo $view_val['mistake_order_reason']; ?>" />
										</td>
								</tr>
								<tr class="tbl-tr cell-tr">
                    ...
                    <input name="cooperation_username" type="text" class="input-common" id="cooperation_username" value="<?php echo $view_val['cooperation_username']; ?>" />
								...
								<tr class="tbl-tr cell-tr">
									...
									<td colspan="3" class="tbl-cell">
										<select name="dept" id="dept" class="select-common" style="width:120px;">
                ...
                <tr class="tbl-tr cell-tr">
                  ...
                  <td colspan="3" class="tbl-cell">
                    <input type="radio" name="sales_type" value="delivery" <?php if($view_val['sales_type'] == 'delivery') {echo "checked";} ?>> 납품
                    <input type="radio" name="sales_type" value="circulation" <?php if($view_val['sales_type'] == 'circulation') {echo "checked";} ?>> 유통
                  </td>
                </tr>
							<?php }else if($_GET['type'] == '3'){?>
								...
								<tr class="tbl-tr cell-tr">
									...
									<td class="tbl-cell">
										<select id="select_sales_user" class="select-common" style="width:120px;">
                    ...
                    <input name="sales_email" type="text" class="input-common" id="sales_email" value="<?php echo $view_val['sales_email']; ?>" />
							...
							<?php }else if($_GET['type'] == '4'){ ?>
								...
								?>
										<tr>
											<td colspan="3" class="tbl-sub-title">
												매입처 정보
											</td>
											<td align="right">
										...
										<tr id="main_insert_field_2_<?php echo $i; ?>" class="tbl-tr cell-tr border-t">
											...
											<td class="tbl-cell" style="padding-right:10px;">
												...
												<input name="main_companyname" type="text" class="input-common" id="main_companyname" value="<?php echo $item2['main_companyname']; ?>" style="width:195px;"/>
												<input name="main_seq" type="hidden" class="input5" id="main_seq" value="<?php echo $item2['seq']; ?>" />
											</td>
											<td class="tbl-title">담당자</td>
											<td class="tbl-cell">
												<select id="select_main_user" class="select-common" onchange="selectMainUser();" style="width:200px;">
													<option value=''>담당자 선택</option>
												</select><br>
												<input name="main_username" type="text" class="input-common" id="main_username" value="<?php echo $item2['main_username']; ?>" style="width:195px;"/>
											</td>
										</tr>
										<tr class="tbl-tr cell-tr">
											<td class="tbl-title">연락처</td>
											<td class="tbl-cell">
												<input name="main_tel" type="text" class="input-common" id="main_tel" value="<?php echo $item2['main_tel']; ?>" onclick="checkNum(this);" onKeyUp="checkNum(this);" style="width:195px;"/>
											</td>
											<td class="tbl-title">이메일</td>
											<td class="tbl-cell">
												<input name="main_email" type="text" class="input-common" id="main_email" value="<?php echo $item2['main_email']; ?>" style="width:195px;" />
											</td>
										</tr>
										<tr>
									...
									?>
										<tr class="main_insert_field_<?php echo $i; ?> tbl-tr cell-tr border-t">
											...
											<td class="tbl-cell" style="padding-right:10px;">
												...
												<input name="main_companyname" type="text" class="input-common" id="main_companyname<?php echo $i;?>" value="<?php echo $item2['main_companyname']; ?>" style="width:195px;"/>
											...
											<td class="tbl-cell">
												...
												<input name="main_username" type="text" class="input-common" id="main_username<?php echo $i;?>" value="<?php echo $item2['main_username']; ?>" style="width:195px;"/>
											</td>
										</tr>
										<tr class="main_insert_field_<?php echo $i; ?> tbl-tr cell-tr">
											<td class="tbl-title">연락처</td>
											<td class="tbl-cell">
                        <input name="main_tel" type="text" class="input-common" id="main_tel<?php echo $i;?>" value="<?php echo $item2['main_tel']; ?>" onclick="checkNum(this);" onKeyUp="checkNum(this);" style="width:195px;"/>
                      </td>
											<td class="tbl-title">이메일</td>
											<td class="tbl-cell">
                        <input name="main_email" type="text" class="input-common" id="main_email<?php echo $i;?>" value="<?php echo $item2['main_email']; ?>" style="width:195px;"/>
                        <img src="<?php echo $misc; ?>img/btn_del0.jpg" onclick='javascript:main_list_del(<?php echo $i; ?>,<?php echo $item2["seq"]; ?>);' style="cursor:pointer;float:right;" />
                      </td>
										</tr>
							...
							<?php }else if($_GET['type'] == '5'){ ?>
								<tr>
									<td colspan="6" height="40" align="left">
										<div>
											...
											<table style="width:70%;">
												...
												<tr>
													...
													<td><input type="button" class="btn-common btn-style2" value="선택적용" onclick="collectiveApplication('product_company',2);" style="width:90px;cursor:pointer;" /></td>
													...
													<td><input type="button" class="btn-common btn-style2" value="선택적용" onclick="collectiveApplication('product_supplier',1);" style="width:90px;cursor:pointer;" /></td>
													...
													<td><input type="button" class="btn-common btn-style2" value="선택적용" onclick="collectiveApplication('product_state',1);" style="width:90px;cursor:pointer;" /></td>
                          <td rowspan="2" style="padding-bottom:10px;">
                            <input type="button" class="btn-common btn-style2" value="일괄적용" onclick="collectiveApplication_all();" style="width:90px;height:60px;" />
												...
												<tr>
													...
													<td>
														<select id="check_product_name" class="select-common" onclick="productSearch('check',this.value);" onchange="product_collective(this);">
															<option value="" selected>제조사를 선택해주세요</option>
														</select>
													</td>
													<td>
                            <input type="button" class="btn-common btn-style2" value="선택적용" onclick="collectiveApplication('product_name',2);" style="width:90px;cursor:pointer;" />
                          </td>
													<td style="font-weight:bold;">장비매출가</td>
													<td>
                            <input type="text" class="input-common" id="check_product_sales" value=0 onclick="numberFormat(this);" onchange="numberFormat(this);product_collective(this);" />
                          </td>
													<td>
                            <input type="button" class="btn-common btn-style2" value="선택적용" onclick="collectiveApplication('product_sales',0);" style="width:90px;cursor:pointer;" />
                          </td>
													<td style="font-weight:bold;">장비매입가</td>
													<td>
                            <input type="text" class="input-common" id="check_product_purchase" value=0 onclick="numberFormat(this);" onchange="numberFormat(this);product_collective(this);" />
                          </td>
													<td>
                            <input type="button" class="btn-common btn-style2" value="선택적용" onclick="collectiveApplication('product_purchase',0);" style="width:90px;cursor:pointer;" />
                          </td>
												</tr>
											</table>
										</div>
                ...
                <tr>
									<td colspan="6" class="tbl-sub-title">제품 정보<span id="check_product_info"></span></td>
								</tr>
								<tr>
									<td colspan="6" >
									...
									<td colspan="6">
										<table id="product_table" style="width:100%;" onchange="filter_reload('product_table',event)" border="0" cellspacing="0" cellpadding="0" class="product_tbl">
											...
											<tr class="row-color1 tbl-tr cell-tr border-t">
												...
												<th class ="apply-filter"><input type="hidden" class="filter_n" value="all">번호</th>
												...
												<th align="center"><img src="<?php echo $misc; ?>img/btn_add.jpg" id="product_add" name="product_add" class="drop" style="cursor:pointer;" /></th>
											</tr>
											</thead>
												...
												?>
													...
													<tr id="product_insert_field_<?php echo $j; ?>" class="tbl-tr cell-tr border-t" >
														<td>
															...
															<input type="checkbox" name="product_row" class="drop" value="<?php echo $i; ?>" />
														</td>
                                ...
                                <option value="" <?php if($item3['product_supplier']==''){echo 'selected';} ?>></option>
														...
														<td>
															<select name="product_type" id="product_type<?php echo $i; ?>" class="select-common" onchange="productSearch(<?php echo $i; ?>);">
                              ...
                              <input name="product_serial" type="text" class="input-common" id="product_serial<?php echo $i; ?>" value="<?php echo $item3['product_serial']; ?>" onkeyup ="commaCheck(this);" />
														...
														<td align="left">
                              <input name="product_sales" type="text" class="input-common" id="product_sales<?php echo $i; ?>" value="<?php if(trim($item3['product_sales']) == ""){echo $item3['product_sales'];}else{echo number_format($item3['product_sales']);} ?>" onclick="numberFormat(this);" onchange="numberFormat(this);product_profit_change(<?php echo $i; ?>,0);" style="text-align:right" />
                              </td>
														<td>
                              <input name="product_purchase" type="text" class="input-common" id="product_purchase<?php echo $i; ?>" value="<?php if(trim($item3['product_purchase']) == ""){echo $item3['product_purchase'];}else{echo number_format($item3['product_purchase']);} ?>" onclick="numberFormat(this);" onchange="numberFormat(this);product_profit_change(<?php echo $i; ?>,0);" style="text-align:right" />
                              </td>
														<td>
                              <input name="product_profit" type="text" class="input-common" id="product_profit<?php echo $i; ?>" value="<?php if(trim($item3['product_profit']) == ""){echo $item3['product_profit'];}else{echo number_format($item3['product_profit']);}?>" style="text-align:right" readonly />
                              </td>
														<td><input name="comment" type="text" class="input-common" id="comment<?php echo $i; ?>" value="<?php echo $item3['comment']; ?>" /></td>
														<td>
                              <?php if($j!=1){
                                echo "<img src='{$misc}img/btn_del0.jpg' onclick='javascript:product_list_del({$j},{$item3["seq"]});' style='cursor:pointer;' /> " ;
                              }?>
                            </td>
													</tr>
												<?php
										...
										<table id="filter_sales" style="min-width:100%;display:none;" cellspacing="0" cellpadding="0" class="product_tbl">
											<colgroup>
												<col width="3%" />
												<col width="3%" />
												<col width="8%" />
										...
										<table style="width:100%;" cellspacing="0" cellpadding="0" class="product_tbl">
											<colgroup>
												...
												<tr id="product_input_place" class="row-color1 tbl-tr cell-tr">
													<td colspan=9></td>
												...
												<tr class="tbl-tr cell-tr">
													<td colspan=9></td>
													<td align="left" >
                            <input name="forcasting_sales" type="text" class="input-common" id="forcasting_sales" value="<?php echo number_format($view_val['forcasting_sales']); ?>" onchange="forcasting_profit_change();" style="text-align:right" readonly />
                          </td>
													<td align="left" >
                            <input name="forcasting_purchase" type="text" class="input-common" id="forcasting_purchase" value="<?php echo number_format($view_val['forcasting_purchase']); ?>" onchange="forcasting_profit_change();" style="text-align:right" readonly />
                          </td>
													<td align="left" >
                            <input name="forcasting_profit" type="text" class="input-common" id="forcasting_profit" value="<?php echo number_format($view_val['forcasting_profit']); ?>" style="text-align:right" readonly />
                          </td>
													<td></td>
													<td></td>
												</tr>
								...
								<tr id="input_point" class="tbl-tr cell-tr border-t">
									...
									<td class="tbl-cell">
                    <select name="division_month" id="division_month" class="select-common" onchange="monthDivision()" style="width:134px;">
                      ...
                      <div id="month" style="float:right">
                      <input type="text" name="monthly_input" class="input-common" value = "<?php echo substr($view_val['division_month'],1)  ?>" id="monthlyInput" onchange="month()" style="width:100px;"></input>
                      개월<div>
									</td>
									<td class="tbl-title">해당월</td>
									<!-- <td colspan="3" align="left" class="t_border" style="padding-left:10px;"> -->
                  <td class="tbl-cell">
										<select name="montly_f_p" id="montly_f_p" class="select-common" style="width:134px;">
										<?php
									...
									<td class="tbl-cell">
										<input name="montly_profit" type="text" class="input-common" id="montly_profit" value="<?php if (substr($view_val['division_month'], 0, 1) === "m") {
										...
										} ?>" style="width:128px;" readonly />
									</td>
								</tr>
								<tr class="tbl-tr cell-tr border-t">
									<td class="tbl-title">최초 매출일</td>
									<td class="tbl-cell">
                    <input name="first_saledate" type="date" class="input-common" id="first_saledate" value="<?php echo $view_val['first_saledate']; ?>" style="width:130px;"/>
                  </td>
									<td class="tbl-title">예상 매출일</td>
									<td class="tbl-cell">
                    <input name="exception_saledate" type="date" class="input-common" id="exception_saledate" value="<?php echo $view_val['exception_saledate']; ?>" style="width:130px;"/>
                  </td>
									<td class="tbl-title">무상보증 종료일</td>
									<td class="tbl-cell">
                    <input name="warranty_end_date" type="date" class="input-common" id="warranty_end_date" value="<?php echo $view_val['warranty_end_date']; ?>" style="width:130px;"/>
                  </td>
								</tr>
				<?php }else{ ?>
				<tr>
					<td colspan="9" class="tbl-sub-title">수주 정보</td>
				</tr>
				<!--시작라인-->
				<tr class="tbl-tr cell-tr border-t">
					<td class="tbl-title">수주여부</td>
					<td class="tbl-cell">
						<select name="complete_status" id="complete_status" class="select-common" style="width:100px;">
  ...
  <tr>
	...
	<tr>
		<td align="right">
      <input type="button" class="btn-common btn-color1" style="cursor:pointer;margin-right:10px;" onclick="popup_close();" value="취소" />
      <input type="button" class="btn-common btn-color2" style="cursor:pointer" onclick="javascript:chkForm();return false;" value="수정" />
		</td>
	</tr>
</table>
<script>
	//select box 검색 기능
	$("#check_product_company").select2({width:'100%'});
  $("select[name=product_company]").select2({width:'100%'});
  ...
  // 고객사 등록되있는건지 확인해서 인풋 보여줄지 말지
	...
	<?php foreach($view_val2 as $item2){ ?>
		...
		$("#select_main_company" + idx).select2({width:'100%'});
		...
		if(idx == "check"){
			...
			$.ajax({
				...
				success: function (data) {
					...
					$("#"+idx+"_product_name").select2({width:'100%'});
				}
			});
		}else{
			...
			$.ajax({
				...
				success: function (data) {
					...
					if(val != undefined){
						...
						// $("#product_name" + idx).val(val).prop("selected",true);
						// $("#product_name" + idx).select2().val(val);
					}
					$("#product_name" + idx).select2({width:'100%'});
				}
			});
		}
	}
