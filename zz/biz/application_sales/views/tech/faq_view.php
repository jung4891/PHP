  ...
  function chkForm( type ) {
   if(type == 1) {
    if (confirm("정말 삭제하시겠습니까?") == true){
     ...
     mform.action="<?php echo site_url();?>/tech/board/faq_delete_action";
     mform.submit();
     return false;
   }
 } else {
  var mform = document.cform;
  mform.action="<?php echo site_url();?>/tech/board/faq_view";
  mform.submit();
  return false;
}
}
                ...
                <td colspan="4" class="tbl-contents"><?php echo $view_val['contents'];?></td>
                   ...
                   if($view_val['file_realname'] != ""){
                      ...
                      for($i=0; $i<count($file); $i++){
                         echo $file[$i];
                         echo "<a href='{$misc}upload/customer/faq/{$file_url[$i]}' download='{$file[$i]}'> <img src='{$misc}img/download.svg' style='width:15px;vertical-align:middle;cursor:pointer;margin:5px 0px 5px 10px;'></a><br>";
                      }
                   }
									 ?>
								</td>
