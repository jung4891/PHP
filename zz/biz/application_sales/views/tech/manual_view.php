  ...
  function chkForm( type ) {
   if(type == 1) {
    if (confirm("정말 삭제하시겠습니까?") == true){
     ...
     mform.action="<?php echo site_url();?>/tech/board/manual_delete_action";
     mform.submit();
     return false;
   }
 } else {
  var mform = document.cform;
  mform.action="<?php echo site_url();?>/tech/board/manual_view";
  mform.submit();
  return false;
}
}
	...
	<div class="dash1-1">
		<table width="95%" height="100%" border="0" cellspacing="0" cellpadding="0" class="dash_tbl1-1">
			<form name="cform" method="get">
				...
				<tr>
					<td align="right">
		<?php if($this->pGroupName == '기술본부' || $this->pGroupName == 'CEO' || $this->pGroupName == '기술연구소') {?>
                ...
                <td colspan="4" class="tbl-contents"><?php echo $view_val['contents'];?></td>
                     ...
                     if($view_val['file_realname'] != ""){
                        ...
                        for($i=0; $i<count($file); $i++){
                           echo $file[$i];
                           echo "<a href='{$misc}upload/customer/manual/{$file_url[$i]}' download='{$file[$i]}'> <img src='{$misc}img/download.svg' style='width:15px;vertical-align:middle;cursor:pointer;margin:5px 0px 5px 10px;'></a><br>";
                        }
                     }
									 ?>
								</td>
