			...
			<form action="./write_update.php" method="post">
				...
				<table id="boardWrite">
					...
					<tbody>
						...
						<tr>
							<th scope="row"><label for="bID2">고객명</label></th>
							<td class="id2">
								<?php
								if(isset($bNo)) {
									echo $row['b_id2'];
								} else { ?>
									<input type="text" name="bID2" id="bID2">
								<?php } ?>
							</td>
						</tr>
						<tr>
							<th scope="row"><label for="bID3">담당자명</label></th>
							<td class="id3">
								<?php
								if(isset($bNo)) {
									echo $row['b_id3'];
								} else { ?>
									<input type="text" name="bID3" id="bID3">
								<?php } ?>
							</td>
						</tr>
						<tr>
							<th scope="row"><label for="bPassword">작업명</label></th>
							<td class="password"><input type="text" name="bPassword" id="bPassword"></td>
						</tr>
						<tr>
							<th scope="row"><label for="bID4">담당SE</label></th>
							<td class="id4">
								<?php
								if(isset($bNo)) {
									echo $row['b_id4'];
								} else { ?>
									<input type="text" name="bID4" id="bID4">
								<?php } ?>
							</td>
						</tr>
						<tr>
