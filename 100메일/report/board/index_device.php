...
<td><a href=./index.php>기술지원보고서 작성</a></td><td><a href=./index_device.php>장비 등록<a/></td><td><a href=./index_buyer.php>사이트 추가</a></td>
	...
	<article class="boardArticle">
		...
		<div id="boardList">
			<table>
				<thead>
					<tr>
						...
						<th scope="col" class="author">제품명</th>
						<th scope="col" class="date">서버</th>
						<th scope="col" class="date">버전</th>
						<th scope="col" class="hit">라이선스</th>
					</tr>
				</thead>
				<tbody>
						<?php
						...
						<tr>
							...
							<td class="title">
								<a href="./view_device.php?bno=<?php echo $row['b_no']?>"><?php echo $row['customer']?></a>
							</td>
							<td class="author"><?php echo $row['produce']?></td>
							<td class="date"><?php echo $row['hardware']?></td>
							<td class="hit"><?php echo $row['version']?></td>
							<td class="hit"><?php echo $row['license']?></td>
						</tr>
						<?php
							}
						}
			...
			<div class="btnSet">
				<a href="./write_device.php" class="btnWrite btn">글쓰기</a>
			</div>
			<div class="paging">
				<?php 
if($allPost!=0)
echo $paging ?>
			...
			<div class="searchBox">
				<form action="./index_device.php" method="get">
					<select name="searchColumn">
						<option <?php echo $searchColumn=='customer'?'selected="selected"':null?> value="customer">고객명</option>
						<option <?php echo $searchColumn=='produce'?'selected="selected"':null?> value="produce">제품명</option>
						<option <?php echo $searchColumn=='hardware'?'selected="selected"':null?> value="hardware">서버명</option>
					</select>
