...
WHERE a.username = '{$uid}' AND a.active = 1";
 ...
 function change_side($width, $user_id){
	 $sql = "SELECT COUNT(*) AS ucount FROM user_setting WHERE user_name = '{$user_id}'";
	 $query = $this->db->query($sql);
	 $num_rows = $query->row();
	 if($num_rows->ucount > 0){
		 $sql = "UPDATE user_setting SET side_width = '{$width}' WHERE user_name = '${user_id}'";
		 $query = $this->db->query($sql);
	 }else{
		 $sql = "INSERT INTO user_setting (user_name, side_width) VALUES ('{$user_id}','{$width}')";
		 $query = $this->db->query($sql);
	 }
 ...
 function get_side($uid){
	 $sql = "SELECT side_width FROM user_setting WHERE user_name = '{$uid}'";
	 $query = $this->db->query($sql);
	 if($query->num_rows() > 0){
