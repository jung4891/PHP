...
for ($i=0; $i < count($folders); $i++) {
 ...
 <div id="main_contents" align="center">
   <div class="sub_div" align="left" style="">
     <span style="font-size:20px;font-weight:bold;">메일분류 규칙 수정</span>
   </div>
   <form id="mform" name="mform" method="post">
     <input type="hidden" id="seq" name="seq" value="<?php echo $modify_data->seq; ?>">
...
<select class="select_basic" name="usertype" id="usertype" onchange="usertype_change(this);">
  <option value="0"<?php if ($modify_data->usertype == 0) {echo " selected";} ?>>특정인</option>
  <option value="1"<?php if ($modify_data->usertype == 1) {echo " selected";} ?>>특정 도메인</option>
</select>
<span id="typespan">
  으로 <?php if ($modify_data->sendtype == 0) {echo " 부터";} ?>
</span>
<select class="select_basic" name="senttype" id="senttype" onchange="sendtype_change(this);">
  <option value="0" <?php if ($modify_data->sendtype == 0) {echo " selected";} ?>>받는 메일</option>
  <option value="1" <?php if ($modify_data->sendtype == 1) {echo " selected";} ?>>보내는 메일</option>
</select>
        </td>
      </tr>
      ...
      <tr>
        ...
        <td align="left">&nbsp;&nbsp;&nbsp;
          <?php if ($modify_data->usertype == 0) {
            $address = explode("@",$modify_data->address);
            $user_part = $address[0];
            $domain_part = $address[1];
          } else{
            $user_part = "";
            $domain_part = $modify_data->address;
          }
          ?>
          <input type="text" class="input_basic input_search" name="user_name" id="user_name" value="<?php echo $user_part; ?>" style="<?php if ($modify_data->usertype == 1) { echo "display:none;"; } ?>width:120px;">@
          <input type="text" class="input_basic input_search" name="domain_name" id="domain_name" value="<?php echo $domain_part; ?>" style="width:120px;">
        </td>
      </tr>
          ...
          <select class="select_basic" name="save_box" id="save_box">
            <?php
                    foreach ($mailbox_tree as $b) {
                      ...
                      $selected = ($id == $modify_data->mailbox) ? " selected" : "";
            ?>
                        <option value= "<?php echo $id?>" <?php echo $selected; ?>><?php echo $text_indented?></option>";
            <?php
                      }
            ?>
          </select>
  ...
  <div class="" align="right" style="width:90%;margin-top:30px;">
    <button type="button" class="btn_basic btn_blue" style="width:60px;height:30px;" name="button" onclick="category_submit();">수정</button>
...
function category_submit(){
  ...
  var act = "<?php echo site_url();?>/option/categorize_modify_action";
